...
> 历史实施计划（2026-09-17），部分约定已被替换。当前契约见 [Agent 记忆架构](../../agent-memory-architecture.md)：episode 按根消息划分；reset 不清空原生/共享上下文；十次门槛仅控制描述统计展示，无论样本量多少都不输出因果结论；身份、观察收据、人格与逐注入配置检查以当前文档和代码为准。
