...
1. `commit(state)` 同步捕获要保存的状态和该次审计队列，串行写临时文件并重命名。状态写成功后才 flush 对应审计；失败时把审计归还队列。事件追加失败由日志健康计数记账，普通协作可以继续。
2. `appendChecked(room, event, validate)` 等待进行中的恢复，再核验当前房间仍是该对象，并同步重验授权。最终检查与事件加入写队列之间没有 `await`；恢复已经完成的旧调用不能继续向新历史写入。需要权威性成功的调用还检查追加结果，不把 `null` 当成成功 id。
3. 同房间恢复和一致快照通过 `runRoom` 排队。恢复先换房间对象、提交状态，再登记日志替换；释放替换等待者后才等待审计 flush，避免 flush 与 restore 互等。替换完成信号的 resolver 不对调用方开放。

`EventLog.read`、`append`、`replace` 在同一实例中共用每房间队列。公共读取取得一致的日志与 head；写入器内部的磁盘读取绕开公共队列，避免把自己的写操作等在自己后面。
...
这些是**单服务实例的并发契约**。状态、JSONL 和 head 是独立文件；没有跨进程锁或两文件崩溃事务。进程中断仍可能留下两者不一致。恢复会有意丢弃快照后的房间状态与事件，且房间快照不覆盖人格、其他房间或原生 Session。全状态备份应在停写时覆盖整个状态目录。
...
manifest 保存配置和哈希，实际群聊注入再次记录配置、人格哈希与记忆可用性。`injection.cost.modelAtDelivery` 是该接收者投递前的运行时模型配置，`modelObservation: "before-delivery"` 明确其观察时点；不是模型执行期间恒定配置的证明。评测比较该观察与 manifest 对应成员的 provider/model/`reasoningEffort`；出现漂移、无效结构或原本已知的配置变为不可读时，排除整个 run。旧日志缺少 `modelAtDelivery` 时标记 `legacy-unverified`，只能做未验证运行内配置的描述。分组包含已记录的推理等级，缺失/null 合并为未记录；provider/model 未知的 run 不与其他 run 合并。源码指纹覆盖显式列出的本地后端依赖闭包，由测试检查；依赖包、宿主隐藏状态、随机性不在内。注释或排版也会改变哈希。旧数据缺少逐注入覆盖信息时标记 `legacy-unverified`，不补造恒定配置的证据。
