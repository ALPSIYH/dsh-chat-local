...
| Belief | 本人用原始观察支持的目前解释，可修订／撤回 | 已验证事实、人格、对另一人的 appraisal |
| Derived memory | 可重建索引和抽取式合并、排序、休眠策略 | 原始证据的替代来源或物理删除 |
...
| `lib/agent-memory.js` | 模型可见文本抽取、旧数据纯函数参考投影、reset 上下文和 600 码元个人摘要 |
| `lib/agent-memory-index.js` | 来源版本／差量索引、本人归属、抽取式合并、控制与信念证据核验、候选和快取预算 |
| `lib/memory-lifecycle.js` | 可逆衰减、生命周期默认配置、三类召回的合计条数和完整 JSON byte 上限 |
| `lib/observation-coverage.js` | 本进程观察收据、失败、非文字、身份歧义及回读排除的已知覆盖指标 |
| `lib/storage-capacity.js` | 单实例写入峰值、提交义务、每人归属事件和可用磁盘空间的预留与拒绝 |
| `lib/cold-log.js` | 整份无损冷档 manifest、内容 hash 与解压上限，供在线和离线读取共用 |
...
| `scripts/persona-longitudinal.mjs` | 真服务连续工作、跨 Session／重启、冻结副本探测、预算和失败分母；不自动宣称人格稳定 |
...
文档及会话事件观察的存储摘录最多 20,000 码元，`chat_memory` 的单个结构化材料摘要最多 2000 码元；个人查询中的每条经历文本最多 2000 码元，并保留截断标记。查询最多 2000 码元，`limit` 为 1–100，限制经历、判断与信念合计数量，完整 JSON 默认最多 64 KiB。`totals` 是本次已检查且符合条件的候选数；候选上限、部分来源和预算省略由 `coverage` 区分，不宣称全历史精确计数。

当前检索使用已验证来源的可重建增量索引，按查询文本匹配、相关对象、釘选和观察时间排序，再按来源与证据 id 确定稳定顺序。观察后背景整理完全相同内容的同来源材料；这是抽取式去重，没有向量或模型语义归纳。默认 30 日半衰期使未釘选的旧记忆休眠；明确相关查询可恢复候选资格，显式抑制则不可绕过。时间戳来自各来源日志，不承诺跨 Session 因果顺序或精确心理时间。日志来源不可读、链校验失败、快取容量或候选上限造成缺口时返回 `partial`；不可读的控制来源不能让抑制内容从其他快取复活。没有收据也可能是记录缺口，不能据此证明 Agent 从未见过。

记忆生命周期策略版本 2 使用同一比较器处理一般读取与固定来源快照：保留现有词项匹配、釘选及相关对象分数；同分时，直接观察材料优先于本人陈述、belief 和 appraisal，再比较观察时间与稳定来源 id。这是预算内的查证来源选择，不表示外部文字必然正确。原生 `assistant/message` 的 `kind: authored` 可明确识别本人陈述；群聊消息仅在事件带有稳定 `authorAgentId` 时计算相对于观察者的 `authoredByObserver`，保留 `kind: message`，也不把他人的消息算成本人的重述。抽取式分组保留这个归属差异，避免同文的他人材料与本人复述合为一条。旧消息缺少稳定作者时不凭名称或措辞猜测归属；当前输入仍可出现在召回中，查询没有排除零匹配候选，釘选与对象加分也未改成纯词项排序。

`/health.audit.memory.coverage` 说明当前采集仅覆盖 live text events，并计数收据失败、非文字事件、未绑定／歧义身份及排除的回读。计数只属于本次服务进程；重启、挂载前原生历史和宿主没有发出的事件不在分母内。
...
`chat_memory_update` 以本人身分和 `operationId` 追加控制或信念事件。pin/unpin 改变召回偏好，suppress/restore 明确停用／恢复材料；原始证据不变。belief 要求 1–8 个原始本人观察，保留内容 hash 和收据，不能以 appraisal 或另一条信念替代原始来源。`supersedes` 修订目前信念，`revoke_belief` 追加撤回。

当前房间、回合与 reset 边界对更新和读取同样适用。再次读到相同材料不使仍存在的原始支持失效；真正 reset、来源替换或原收据消失则不能借新观察复活旧信念。群聊固定日志前缀的索引视图同样继承已授权跨来源控制。完整操作契约及配置见 [记忆生命周期](memory-lifecycle.md)。
...
| 本人经历/判断/信念摘要 | 600 UTF-16 码元 | 来源/证据、所选条目、截断和省略原因 |
...
暖个人读取使用已验证事件视图和增量索引，不再逐次全量重放；判断按身份／对象增量更新。索引有全域和每人预算，淘汰会披露缺口；重启、首次读取、来源替换或快取淘汰仍会整份读取来源，派生数据预算也不等于实际 RSS 上限。没有语义自动归纳或矛盾推理，不会把多个观察者的私人记忆合成共同事实。

日志验证快取及个人索引分别限额，原始历史、人格旧版和替换收据仍随历史增长。所有本实例管理的写入共用容量预留，保留既有提交义务的完成空间，容量不足拒绝新工作。管理 API 可把整份来源无损压缩为带 manifest 的冷档；追加前整份 thaw，cleanup 只清理不再引用的压缩副本，没有自动分段或保留期删除。默认值和精确边界见 [容量与冷档](memory-lifecycle.md#容量與冷檔)。

持久化比仅写缓冲区增加同步 I/O；回覆计时从实际发送前开始，准备和留档不会消耗成员的回覆时间。关闭会结束本地等待、保存投递终态并等待记忆刷新、人格与容量队列完成，但不声称取消了已发给外部系统的工作。
...
node --test test/agent-memory-index.test.js test/memory-index-independent.test.js test/memory-lifecycle-service.test.js
...
长历史回归覆盖 15 万事件的派生、判断、分段与统计，以及 15 万消息的真实冷启动；冷启动路径仍整份读入，不代表无限规模或固定内存开销。子进程 SIGKILL 测试覆盖状态写后事件前、事件写后 head 前、快照恢复中断、恢复再次中断以及重叠提交。注入的磁盘故障还覆盖部分写入、待恢复记录损坏、重放冲突、检查点失败及目录同步错误。人格文件测试覆盖部分写入、历史哈希冲突、符号链接和非法 UTF-8；[纵向人格工具](persona-longitudinal.md) 则保存持续工作轨迹、隔离 probe 和实际注入证据，区分事实检查与需人工盲评的人格判断。测试通过不能认证未覆盖的历史完整性、硬件失效或长期人格稳定性。
