...
/**
 * Verify a snapshot before it is allowed to overwrite anything. The content
 * hash proves the file was not edited after export, and the chain check proves
 * the log inside it is a log at all: a snapshot built from a log with a missing
 * or reordered tail would otherwise restore straight over the live one.
 */
export function validateSnapshot(parsed) {
  ...
  if (!chain.ok) return { ok: false, reason: `event chain does not verify at index ${chain.brokenAt}` };
