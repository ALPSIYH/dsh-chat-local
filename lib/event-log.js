...
export class EventLog {
   ...
   * being awaited by their caller, so shutdown has to wait for them too:
   * otherwise removing the state tree races a write into `events/`.
   */
  async drain() {
    // Two passes: the first settles what is queued now, the second covers an
    // append queued by a continuation of the first. After `close` no producer
    // remains, so this terminates.
