...
import { observationOwner } from "./storage-capacity.js";
import { coldManifest, coldManifestPath, contentHash, readColdLog, readColdManifest } from "./cold-log.js";
...
export class EventLog {
  ...
  constructor(statePath, { capacity, cacheBytes = 256 * 1024 * 1024 } = {}) {
    if (!Number.isSafeInteger(cacheBytes) || cacheBytes < 0) throw new TypeError("cacheBytes must be a non-negative integer");
    this.#statePath = statePath; this.#capacity = capacity; this.#cacheBytes = cacheBytes;
  }

  #storage(options, operation) { return this.#capacity ? this.#capacity.run(options, operation) : operation(); }
  clearCache() { const rooms = this.#cache.size; this.#cache.clear(); this.#evictions += rooms; return rooms; }
  #trimCache() {
    let total = [...this.#cache.values()].reduce((sum, value) => sum + value.bytes, 0);
    for (const [id, value] of this.#cache) {
      if (total <= this.#cacheBytes) break;
      this.#cache.delete(id); total -= value.bytes; this.#evictions += 1;
    }
  }
  ...
  health() {
    return { appended: this.#appended, failed: this.#failed, lastError: this.#lastError,
      ...
      recoveredOperations: this.#recoveredOperations, fullReads: this.#fullReads, viewHits: this.#viewHits,
      cacheRooms: this.#cache.size, cacheSourceBytes: [...this.#cache.values()].reduce((sum, value) => sum + value.bytes, 0),
      cacheLimitBytes: this.#cacheBytes, cacheEvictions: this.#evictions };
  }
  ...
  async #signature(roomId) {
    const parts = [];
    const logPath = eventLogPath(this.#statePath, roomId), manifest = await readColdManifest(logPath);
    const paths = [logPath, eventLogHeadPath(this.#statePath, roomId), coldManifestPath(logPath)];
    if (manifest) paths.push(join(dirname(logPath), manifest.archive));
  ...
  async #recoverDisk(roomId) {
    ...
    return this.#storage({peakBytes: Buffer.byteLength(JSON.stringify(intent)) * 4 + Buffer.byteLength(JSON.stringify(await this.#receipts(roomId))) + 8192, recovery:true, claimId:intent.operationId ?? intent.event?.provenance?.operationId}, async () => {
  ...
  async #verified(roomId) {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      ...
      if (cached?.signature === signature) {
        this.#cache.delete(roomId); this.#cache.set(roomId, cached);
        return cached;
      }
      this.#cache.delete(roomId);
      ...
      this.#fullReads += 1;
      requireChain(events);
      if (signature !== await this.#signature(roomId)) continue;
      const operations = new Map(), operationCounts = new Map();
      for (const [index, event] of events.entries()) {
        ...
        operations.set(id, event); operationCounts.set(id, index + 1);
      }
      const value = { signature, generation: randomUUID(), events,
        head: events.at(-1)?.hash ?? null, at: atSeed(events.at(-1)), bytes, needsSeparator, operations, operationCounts };
      this.#cache.set(roomId, value); this.#trimCache();
      return value;
    }
    throw new Error("event log changed continuously while verifying; retry the read");
  }

  /** Authority checked on every call. Only own durable appends preserve a
   * generation; external changes, replacement and eviction require a rebuild.
   * Returned event objects and arrays are immutable, never a live cache array.
   */
  readView(roomId, { after } = {}) {
    return this.#enqueue(roomId, async () => {
      const cached = await this.#verified(roomId);
      const revision = Object.freeze({generation:cached.generation, count:cached.events.length, head:cached.head});
      const appendOnly = after?.generation === revision.generation && Number.isSafeInteger(after.count)
        && after.count >= 0 && after.count <= revision.count
        && after.head === (cached.events[after.count - 1]?.hash ?? null);
      const events = appendOnly && after.count === revision.count ? EMPTY_EVENTS
        : Object.freeze(cached.events.slice(appendOnly ? after.count : 0));
      if (appendOnly) this.#viewHits += 1;
      return Object.freeze({revision, baseRevision:appendOnly ? Object.freeze({...after}) : null, appendOnly, events, verified:true});
    });
  }

  hasPublished(roomId, operationIds, revision) {
    return this.#enqueue(roomId, async () => {
      const cached = await this.#verified(roomId);
      return cached.generation === revision?.generation
        && operationIds.every(id => (cached.operationCounts.get(id) ?? Infinity) <= revision.count);
    });
  }
   ...
   */
  append(roomId, input, operationId, { recovery = false } = {}) {
    ...
    return this.#enqueue(roomId, async () => {
      ...
      try {
        ...
        await this.#thawDisk(roomId, recovery);
        const cached = await this.#verified(roomId);
        ...
        await mkdir(dirname(path), { recursive: true, mode: 0o700 });
        await assertRegularFile(path);
        await this.#storage({peakBytes: Buffer.byteLength(line) * 3 + 4096, recovery, claimId: operationId, agentId:observationOwner(event), agentBytes:Buffer.byteLength(line)}, async () => {
          ...
          this.#appended += 1;
          await this.#clearIntent(roomId);
          await this.#capacity?.noteAppend(roomId, event, Buffer.byteLength(line));
        });
        ...
        const remembered = immutable(structuredClone(event));
        cached.events.push(remembered);
        if (operationId !== undefined) { cached.operations.set(operationId, remembered); cached.operationCounts.set(operationId, cached.events.length); }
        this.#trimCache();
        this.#lastError = null;
        return event;
      } catch (error) {
  ...
  replace(roomId, events, operationId, { recovery = false } = {}) {
    ...
    return this.#enqueue(roomId, async () => {
      ...
      const receiptBytes = Buffer.byteLength(JSON.stringify(await this.#receipts(roomId)));
      await this.#storage({peakBytes: Buffer.byteLength(JSON.stringify(intent)) * 3 + receiptBytes + 4096, recovery, claimId:operationId}, async () => {
  ...
  async #forgetCold(roomId) {
    const path = eventLogPath(this.#statePath, roomId), manifest = await readColdManifest(path);
    if (!manifest) return;
    // The replacement raw file and its head are durable before withdrawing the
    // manifest. A crash before this point is guarded by the replace intent.
    await rm(coldManifestPath(path));
    await syncPath(dirname(path));
    await rm(join(dirname(path), manifest.archive), {force:true});
    await syncPath(dirname(path));
  }

  async #thawDisk(roomId, recovery = false) {
    const path = eventLogPath(this.#statePath, roomId);
    if (!await readColdManifest(path)) return false;
    // Thaw withdraws the cold authority. It must prove the same complete
    // source as a read: byte hash, chain, manifest range and head anchor.
    // Checking only the compressed bytes would silently erase a bad manifest.
    const verified = await this.#verified(roomId), cold = await readColdLog(path);
    if (!cold || await this.#signature(roomId) !== verified.signature) throw new Error("cold log changed before thaw; retry maintenance");
    await this.#storage({peakBytes:cold.bytes.length + 4096, recovery}, async () => {
      if (await this.#signature(roomId) !== verified.signature) throw new Error("cold log changed before thaw; retry maintenance");
      await atomicText(path, cold.bytes);
      const current = await this.#verified(roomId);
      if (current.head !== verified.head || current.events.length !== verified.events.length) throw new Error("cold log changed during thaw; its manifest remains authoritative");
      if ((await readColdManifest(path))?.checksum !== cold.manifest.checksum) throw new Error("cold log changed during thaw; its manifest remains authoritative");
      // Until the marker is removed either cold or hot carries identical
      // evidence. A failed cleanup is retryable, never a partial history.
      await this.#forgetCold(roomId);
    });
    this.#cache.delete(roomId);
    return true;
  ...
  /** Explicit host maintenance; no source is deleted without a verified,
   * durable lossless replacement. The empty .jsonl keeps legacy discovery.
   * Whole-log compression trades disk space for a bounded thaw peak on append.
   */
  archive(roomId, {maxBytes = 256 * 1024 * 1024} = {}) {
    if (!Number.isSafeInteger(maxBytes) || maxBytes < 0) return Promise.reject(new TypeError("maxBytes must be a non-negative safe integer"));
    return this.#enqueue(roomId, async () => {
      const cached = await this.#verified(roomId);
      const path = eventLogPath(this.#statePath, roomId), existing = await readColdLog(path);
      if (existing) {
        await this.#storage({peakBytes:0, recovery:true}, async () => { await writeFile(path, "", {mode:0o600}); await syncPath(path); });
        this.#cache.delete(roomId);
        return {roomId,archived:true,alreadyArchived:true,sourceBytes:existing.bytes.length,
          compressedBytes:(await stat(existing.archive)).size};
      }
      if (cached.bytes > maxBytes) return {roomId,archived:false,reason:"archive-size-limit",sourceBytes:cached.bytes};
      const raw = await this.#optional(path) ?? Buffer.alloc(0);
      if (await this.#signature(roomId) !== cached.signature) throw new Error("event log changed before archival; retry maintenance");
      const compressed = await compress(raw, {level:9});
      const archive = `${basename(path)}.cold-${randomUUID()}.gz`;
      const manifest = coldManifest({version:1,codec:"gzip",archive,bytes:raw.length,hash:contentHash(raw),
        count:cached.events.length,head:cached.head});
      const text = `${JSON.stringify(manifest)}\n`;
      await this.#storage({peakBytes:compressed.length + Buffer.byteLength(text) + 4096}, async () => {
        if (await this.#signature(roomId) !== cached.signature) throw new Error("event log changed before archival publication; retry maintenance");
        const archivePath=join(dirname(path),archive);
        await atomicText(archivePath, compressed);
        // Verify generated bytes before the manifest can become authoritative.
        const decoded = await decompress(await readFile(archivePath), {maxOutputLength:Math.max(1, raw.length)});
        if (!decoded.equals(raw)) throw new Error("cold log compression verification failed");
        if (await this.#signature(roomId) !== cached.signature) throw new Error("event log changed during archival; source was not replaced");
        await atomicText(coldManifestPath(path), text);
        this.#cache.delete(roomId);
        await writeFile(path, "", {mode:0o600});
        await syncPath(path);
      });
      return {roomId,archived:true,alreadyArchived:false,sourceBytes:raw.length,compressedBytes:compressed.length};
    });
  }

  /** Remove only unreferenced generated compression copies after proving the
   * current source complete. The current manifest and raw evidence stay put.
   * Reads never perform this cleanup; interrupted maintenance is explicit.
   */
  cleanupArchives(roomId) {
    return this.#enqueue(roomId, async () => {
      await this.#verified(roomId);
      const path = eventLogPath(this.#statePath, roomId), manifest = await readColdManifest(path);
      let names;
      try { names=await readdir(dirname(path)); } catch(error) { if(error.code==='ENOENT')return {roomId,removed:0}; throw error; }
      const prefix=`${basename(path)}.cold-`;
      const obsolete=names.filter(name=>name.startsWith(prefix) && /^[a-f0-9-]+\.gz$/.test(name.slice(prefix.length)) && name!==manifest?.archive);
      await this.#storage({peakBytes:0,recovery:true},async()=>{
        for(const name of obsolete)await rm(join(dirname(path),name),{force:true});
        if(obsolete.length)await syncPath(dirname(path));
      });
      return {roomId,removed:obsolete.length};
    });
  }
  ...
  thaw(roomId) {
    return this.#enqueue(roomId, async () => {
      await this.#recoverDisk(roomId);
      return {roomId,thawed:await this.#thawDisk(roomId)};
    });
  }
  ...
  async #readDisk(roomId, details = false) {
    if (await this.#intent(roomId)) throw new Error("event log recovery required: unfinished write intent; run service recovery before reading");
    const path = eventLogPath(this.#statePath, roomId);
    const cold = await readColdLog(path);
    const text = cold ? cold.bytes.toString("utf8") : await this.#optional(path, "utf8") ?? "";
    const events = parseEvents(text);
    if (cold && (cold.manifest.count !== events.length || cold.manifest.head !== (events.at(-1)?.hash ?? null))) {
      throw new Error("cold log event range does not match its manifest");
    }
