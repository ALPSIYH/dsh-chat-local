...
export class EventLog {
   ...
   *
   * Public reads use the same queue. Cold writer initialization uses the private
   * disk reader instead, so a queued operation never waits on itself. Every
   * stored tail is wrapped in `.catch`, so a failed operation cannot poison it.
   */
  async drain() {
  ...
  async #lastHashFor(roomId) {
    ...
    if (!this.#lastHash.has(roomId)) {
      ...
      const existing = await this.#readDisk(roomId).catch((error) => {
        if (error?.code === "ENOENT") return [];
        throw error;
      });
      this.#lastHash.set(roomId, existing.at(-1)?.hash ?? null);
  ...
  /**
   * Read one coherent room log, ordered with this writer's appends/replaces.
   * Registering the whole read, rather than draining before reading, keeps a
   * later write from changing the anchor between the two file reads. This is
   * same-instance ordering, not a cross-process lock or crash transaction.
   *
   ...
   * "written before anchors existed". An anchor may lag after an interrupted
   * write; it must occur somewhere in the returned chain.
   *
  ...
  read(roomId) {
    const previous = this.#tails.get(roomId) ?? Promise.resolve();
    const next = previous.then(() => this.#readDisk(roomId));
    this.#tails.set(roomId, next.catch(() => {}));
    return next;
  }

  /** Already inside the room queue, or initializing a cold queued writer. */
  async #readDisk(roomId) {
    const path = eventLogPath(this.#statePath, roomId);
    await assertRegularFile(path);
