...
import { mkdir, lstat, readFile, writeFile, rename, link, unlink, open } from 'node:fs/promises';
...
async function durableWrite(path, markdown) {
...
async function publishNew(path, markdown) {
  ...
  try {
    await durableWrite(temporary, markdown);
    try { await link(temporary, path); await syncFile(dirname(path)); }
...
export class AgentPersonas {
  ...
  #closed = false;
  #track(run) {
    if (this.#closed) return Promise.reject(new Error('persona service is closed'));
    const operation = run(); this.#active.add(operation);
    void operation.then(() => this.#active.delete(operation), () => this.#active.delete(operation));
    return operation;
  }
  async close() { this.#closed=true; await Promise.allSettled([...this.#active]); }
  async drain() { await Promise.allSettled([...this.#active]); }
  read(agent) { return this.#track(() => this.#read(agent)); }
  save(agent, input) { return this.#track(() => this.#save(agent, input)); }
  constructor(statePath, {capacity} = {}) { this.directory = join(dirname(statePath), 'agents'); this.capacity = capacity; }
  write(peakBytes, operation) { return this.capacity ? this.capacity.run({peakBytes}, operation) : operation(); }
  ...
  async #read(agent) {
    ...
    try { await lstat(path); }
      ...
      await this.write(Buffer.byteLength(template(agent)) * 2 + 8192, async () => {
        await managedDirectory(this.directory);
        await managedDirectory(dirname(path));
        await publishNew(path, template(agent));
      });
    }
    for (const directory of [this.directory, dirname(path)]) {
  ...
  async #save(agent, { markdown, expectedHash } = {}) {
    ...
    if (Buffer.from(markdown, 'utf8').toString('utf8') !== markdown) throw new Error('persona Markdown must be well-formed Unicode; unpaired UTF-16 surrogates were not replaced');
    ...
    const operation = previous.then(async () => {
      ...
      return this.write(Buffer.byteLength(current.markdown) * 2 + Buffer.byteLength(markdown) + 8192, async () => {
      if (fingerprint(await regularMarkdown(path)) !== expectedHash) throw Object.assign(new Error('persona changed; read and compare before saving'), {status:409});
