 ...
 *
 * There is one source for every digest term *and* for the algorithm that renders
 * them. The injected text is assembled here by the functions below, and
 * `injectionConfigFor` hashes both those constants and a fingerprint of those
 * functions' own source, so neither a term nor the algorithm can change the
 * injected text without changing the hash an experiment's comparability rests
 * on. `INJECTION_CONFIG_VERSION` remains the declared, human-readable name of
 * the algorithm's version: the fingerprint makes a change visible, the version
 ...
 * text, in the order they are fingerprinted. Every one of them is listed by
 * name: a helper left out here could change the bytes a member reads without
 * changing the hash, which is exactly the hole this list exists to close.
 */
...
/**
 * A digest of the rendering algorithm's own source text.
 *
 ...
 * so two runs that injected different text could be pooled as comparable. No
 * convention can close that hole, because the hole is that the algorithm is not
 * part of the hash. This makes it part of the hash: the source text of every
 * renderer is fingerprinted, so any edit to the algorithm — a literal, a
 * branch, even a comment inside one of these functions — changes the hash and
 * splits the runs instead of silently merging them.
 *
 * The cost of that strictness is honest: a reformatting that changes nothing
 * observable also splits the groups. The alternative is a hash that cannot tell
 * two different renderings apart, which is the failure this whole comparability
 ...
 * constants. `algorithm` is the fingerprint of the rendering functions;
 * `relationshipVersion` is the version of the counter derivation those
 * functions render, because a new derivation can put different numbers — and so
 * different text — behind the same terms.
 *
...
/**
 * Whether one run holds an interaction the evaluation can read.
 *
 ...
 * text injected into a member's turn, and `injection.cost` is the record that
 * such a turn happened and what it carried, so at least one of those is the
 * threshold between "a run was started" and "a run has data".
 *
