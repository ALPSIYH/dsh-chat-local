...
export class RoomJournal {
  ...
  async #write(state, pending, committed = () => {}) {
  ...
  #publish(operation) {
    ...
    const publishing = (async () => {
      try {
        ...
        if (operation.kind === "replace") await this.#events.replace(operation.roomId, operation.events, operation.id);
        else if (await this.#events.appendOnce(operation.roomId, operation.event, operation.id) === null) {
  ...
  #checkpoint() {
    const checkpoint = this.writeTail.then(async () => {
      ...
      try { await this.#write(this.#lastState, pending); }
  ...
  record(room, event, validate = () => {}) {
    ...
    const publish = () => {
      ...
      validate();
      return this.#events.append(room.id, input);
    };
  ...
  async appendChecked(room, event, validate = () => {}) {
    ...
    if (this.#currentRoom(room.id) !== room) throw new Error("room operation was superseded by a restored snapshot");
    return this.record(room, event, validate);
  }
