...
export class RoomJournal {
  ...
  #beginCommit(state) {
    ...
    // A later save may already be queued when an earlier write fails. Its
    // snapshot contains those earlier changes, so it must capture their audit
    // eligibility now as well. Waiting to requeue after failure loses this
    // obligation until some unrelated future save happens.
    ...
    try {
      ...
      this.pendingAudits.unshift(...queued);
      throw error;
    }
  ...
  #flush(audited) {
    // Eligibility belongs to a successful snapshot, not to the latest live
    // value. A later save can help publish an earlier committed transition;
    // it cannot retract that fact merely because its own snapshot has moved on.
    for (const { entry, inSnapshot } of audited) {
      if (inSnapshot) this.#committedAudits.add(entry);
    }
    const flush = (async () => {
      for (const { entry, inSnapshot } of audited) {
        if (!this.#unpublished.has(entry)) continue;
        if (!inSnapshot && !this.#committedAudits.has(entry)) { this.#finishAudit(entry); continue; }
        ...
        if (!this.#unpublished.has(entry)) continue;
        if (this.#currentRoom(entry.room?.id) !== entry.room || !entry.stillValid()) {
          this.#finishAudit(entry);
          continue;
        }
        const event = entry.build();
        // The append is registered synchronously. Remove the shared obligation
        // before awaiting it, so overlapping successful saves publish it once.
        this.#finishAudit(entry);
        await this.record(entry.room, event);
      }
    })();
