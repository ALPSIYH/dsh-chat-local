...
export class AgentMemoryIndex {
  ...
  #putLeaf(agentId, item, aliases = []) {
    ...
    const groupId = key(item.sourceRoomId, item.kind, item.contentHash ?? memoryContentHash(item.claim), item.targetAgentId ?? null);
  ...
  recall({ agentId, sourceRoomIds, context, query = '', targetAgentIds = [], limit = 24,
    ...
    if (sourceOverrides.length) {
      ...
      try {
        ...
        const terms = memoryTerms(query), score = item => terms.reduce((n, term) => n + Number(String(item.text ?? item.claim).toLowerCase().includes(term)), 0)
          + (item.pinned ? 10 : 0) + (targetAgentIds.includes(item.targetAgentId) ? 10 : 0);
        candidates.sort((a, b) => score(b) - score(a) || latest(a, b));
    ...
    for (const original of selected.values()) {
      ...
      const score = activation.matches + (card.pinned ? 10 : 0) + (targetAgentIds.includes(card.targetAgentId) ? 10 : 0);
      if (consolidation) candidates.push({ ...item, memoryState: activation.state, _score: score });
        ...
        const ownActivation = memoryActivation({ ...raw, pinned: this.#control(agent, leaf, new Set(_policySourceRoomIds)).pinned }, { now, query, mode, lifecycle: decay, halfLifeMs, minActivation });
        if (ownActivation.eligible) candidates.push({ ...raw, memoryState: ownActivation.state, _score: score });
      }
    }
    candidates.sort((a, b) => b._score - a._score || latest(a, b));
