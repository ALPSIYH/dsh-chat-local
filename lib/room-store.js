...
import {
  ...
  RELATIONSHIP_DIGEST_MAX_CHARS, relationshipDigest,
 ...
 */
const GROUP_INTERNAL_TOOLS = new Set(["chat_send", "chat_rooms", "chat_memory", "chat_relationships", "chat_appraise", "chat_charter_propose", "chat_charter_review", "chat_work", "chat_read_document", "chat_manage"]);
...
/**
 * The log replaces in flight, keyed by room id and holding at most one per room.
 *
 * A restore replaces the whole log of one room, and an audit append registered
 * before that replace is erased by it. The restore therefore publishes its
 * replace here while the swap and its state write are in flight, and a flush
 * waits on whatever this registry holds for an entry's room before deciding
 * anything about the entry. The wait is keyed on the room id because the object
 * an entry belongs to may be the one the restore detached: only a replace of
 * that same id can bring it back, so only that same id can settle the question.
 *
 * Two properties are structural here rather than conventions, because the wait
 * loop depends on both:
 *
 * - `release` drops the entry before it resolves it, inside this one method, and
 *   the promise's resolver never leaves the class. A registry entry that had
 *   resolved while still registered would be found again by the flush's
 *   re-lookup, and the loop would spin in microtasks forever.
 * - `open` refuses a second replace for a room that already has one. Restores
 *   are serialised per room id (see `#takeRestoreLock`), so this never fires in
 *   the product; it is an assertion that keeps the lookup single-valued, and
 *   with it the room-keyed wait cannot have two gates to choose between.
 */
class PendingLogReplace {
  #entries = new Map();

  get size() { return this.#entries.size; }

  /** The replace in flight for one room, or null when the room's log is quiet. */
  get(roomId) { return this.#entries.get(roomId) ?? null; }

  /** Publish one restore's replace. Returns the entry `release` takes back. */
  open(roomId) {
    if (this.#entries.has(roomId)) throw new Error(`a log replace is already in flight for room ${String(roomId)}`);
    const entry = { roomId, promise: null, resolve: null };
    entry.promise = new Promise((resolve) => { entry.resolve = resolve; });
    this.#entries.set(roomId, entry);
    return entry;
  }

  /** Drop an entry and wake its waiters, in that order and only in this method. */
  release(entry) {
...
export class DshChatLocalService {
  constructor(ctx, config = {}) {
    ...
    // The flush of the last save's claimed audits. It is tracked separately from
    // `saveTail` because the state write is durable before its audits are
    // appended, and a reader of the log must be able to join the later half too.
    ...
    this.pendingAudit = [];
    // The log replace a restore has published but not yet registered on the
    // log's own tail, keyed by room id and holding at most one per room. A flush
    // waits on it before asking anything about an entry: an append the replace
    // would erase is then ordered after it, and an entry for the object the
    // replace detached is refused only once the restore has settled. Releasing a
    // replace never waits for a flush — the restore registers the replace and
    // releases the entry first, and only then joins the flush — so every wait a
    // flush makes is on a path that cannot lead back to that flush.
    this.pendingLogReplace = new PendingLogReplace();
    // One restore at a time per room id, so the registry above is single-valued
    // for a room and no second replace can be in flight beside the first.
    this.restoreTails = new Map();
  ...
  /**
   * Persist the state and then append the audits this snapshot made true.
   *
   * This is the ordinary path. `restoreFromSnapshot` needs the two halves —
   * the write and the flush — separately, because it must register its log
   * replace between them; it therefore calls `#beginSave` and `#flushAudits`
   * itself rather than this method. See the restore for why that ordering is the
   * one that makes every flush's wait acyclic.
   */
  async #save() {
    const { claimed, audited, write } = this.#beginSave();
    try {
      await write;
    } catch (error) {
      // Nothing reached disk, so these audits must wait for a save that does.
      // The raw entries are re-queued, not the claimed copies: a retry has to
      // decide `inSnapshot` again against the state it serialises, not inherit
      // the verdict of the write that failed.
      this.pendingAudit.unshift(...claimed);
      throw error;
    }
    await this.#flushAudits(audited);
  }

  /**
   * Claim this state's audits and chain the write that makes them true.
   *
   * Claiming here rather than in the flush keeps a state change queued during
   * the write from being audited by a save whose snapshot does not yet contain
   * it. The claim also settles, synchronously and against the state just
   * serialised, whether this snapshot still holds the fact each event names
   * (`inSnapshot`). That question cannot be deferred to the flush: the objects
   * the events describe are mutated in place, so by the time the write lands a
   * delivery may have moved on, and a transition this save really did write
   * would be dropped from the log forever.
   *
   * The caller owns the failure: when `write` rejects it must re-queue
   * `claimed` and may not flush, because nothing reached disk.
   */
  #beginSave() {
    ...
    const claimed = this.pendingAudit.splice(0);
    // The claim is fallible — `inSnapshot` is supplied by the queuing caller —
    // and the queue has already been spliced, so it is decided inside a guarded
    // region: a throw re-queues the whole batch for a later save instead of
    // discarding every claimed entry and skipping the write entirely.
    let audited;
    try {
      audited = claimed.map((entry) => ({
        entry,
        inSnapshot: entry.inSnapshot ? entry.inSnapshot() : true
      }));
    } catch (error) {
      this.pendingAudit.unshift(...claimed);
      throw error;
    ...
    return { claimed, audited, write: operation };
  }

  /**
   * Append the audits a landed write made true.
   *
   * The flush is its own promise, published before it runs: a reader that only
   * joined the state write would otherwise be able to return a log missing an
   * append this save still owes. Settling on it is what makes "already owed"
   * and "already appended" the same claim. What is published is a promise that
   * cannot reject: the state is durable by the time this runs, and a later
   * `settledAudit` is a read that must report the log rather than inherit a
   * failure from a save it never made. The caller that awaited the write still
   * sees the failure through the returned promise.
   *
   * A flush waits on log replaces, never the other way around: every replace is
   * released by its restore on a path that joins no flush (see
   * `restoreFromSnapshot`), so a wait here always settles.
   */
  #flushAudits(audited) {
    const flush = (async () => {
      for (const { entry, inSnapshot } of audited) {
        // A state change the snapshot this save wrote never held, or one rolled
        // back before it was persisted, must never be audited: the first was
        // decided when the snapshot was serialised, the second is re-checked
        // here against the state that reached disk.
        if (!inSnapshot) continue;
        // Every log replace in flight for this entry's room is waited out before
        // anything is asked about the entry, because both directions of the
        // central rule below need that replace to have settled first.
        //
        // The ordering half: a restore replaces a room's whole log, so an
        // append registered before the replace is erased by it — and identity
        // alone cannot order the two, because a save claimed after the swap
        // passes the identity check, while the restore joins that save's write
        // but not its flush, so the append can still land first.
        //
        // The settlement half: an entry for the object a restore detached
        // cannot be refused merely because the detach has happened. The detach
        // is the swap, and the swap is durable only once the restore's own
        // state write lands; when that write fails the restore puts the object
        // back, it is the live room again, and this entry's fact is exactly the
        // state on disk. Deciding on the swap alone would drop the entry with
        // no re-queue and no trace, and the log would under-describe
        // `rooms.json` — the loss this whole rule exists to prevent.
        //
        // Identity is therefore asked only after the wait, and the wait is
        // repeated until the room's log is quiet: a replace published while this
        // flush waited is one whose erase this append must also follow. The
        // lookup is single-valued by construction — one entry per room id, which
        // `PendingLogReplace.open` refuses to double and the restore lock keeps
        // from arising — and an entry is dropped from the registry before its
        // promise resolves, so this loop can never re-find the replace it just
        // waited on. Each pass therefore waits on a *newer* replace of that room,
        // and each restore releases the one it published on a path that joins no
        // flush, so the loop always reaches a quiet room.
        //
        // Unlike the round that preceded it, no replace is exempt. A restore's
        // own flush waits here on its own replace too, and that is safe rather
        // than circular: the restore registers the replace and releases the entry
        // *before* it joins this flush, so the wait is on work already done by
        // the time it is made.
        let replace = this.#pendingLogReplaceFor(entry.room);
        while (replace) {
          await replace.promise;
          replace = this.#pendingLogReplaceFor(entry.room);
        }
        // The identity of the room object the fact belongs to: it is the only
        // discriminator there is, because a restore keeps the room id and
        // changes the object. An entry queued against the object a settled
        // restore discarded is refused here; one whose restore rolled back is
        // the live room again and passes.
        if (!this.#roomStillLive(entry.room)) continue;
        if (!entry.stillValid()) continue;
  ...
  async settledAudit() {
    ...
    for (let round = 0; round < 32; round += 1) {
      const owed = this.auditFlush;
      await this.saveTail;
      await owed;
      // A save that claimed entries after this pass read the queue owes a flush
      // this pass has not joined yet; only a quiet, empty queue is settled.
      if (owed === this.auditFlush && this.pendingAudit.length === 0) break;
    }
    await this.saveTail;
  ...
  async appraisals(roomId) {
    ...
    return effectiveAppraisals(events, room.id, room.tick ?? 0);
  }
  ...
  async relationshipRow(roomId, sessionId) {
    ...
    let appraisals;
    try { appraisals = effectiveAppraisals(await this.#readEvents(room.id), room.id, room.tick ?? 0); }
    catch { appraisals = {}; }
   ...
   *
   * - **Evidence is mandatory and must exist.** `evidenceEventIds` must be a
   *   non-empty list of ids that this room's own log actually carries. The ids
   *   are checked against a `Set` built from one drained read of the log, never
   *   by rescanning the log per id. What counts as an id is the plan's open
   *   question, and the answer is taken from the log rather than guessed: an
   *   envelope's own `id`, the `messageId` a `message.created` carries, and the
   *   `entryId` a `ledger.transition` carries — every identifier an agent can
   *   actually read back through `chat_memory`. A charter proposal id is in that
   *   domain through its `entryId`, which the propose transition sets to the
   *   proposal id; nothing emits a `charter.*` event, so a proposal's id is
   *   never carried as a `payload.proposalId`. An id that exists nowhere in
   *   this room's log is refused, so an appraisal cannot cite a conversation
   *   that never happened. Refusing (rather than dropping the unknown ids) is
   *   deliberate: silently narrowing a citation would record a different
   ...
   * - **What the writer stamps, and how the append lands.** `validFrom` is the
   *   room's current tick, so the statement is in force from the tick it was made
   *   on. The append is a direct `#record` (`#recordAppraisal` / `#revokeAppraisal`
   *   call it and hand back the event's id as `appraisalId`), **not** an entry in
   *   the audit queue the other room-state writes go through: an appraisal is not
   *   a projection of room state, so there is no save whose snapshot has to make
   *   it true, and the caller learns the id as soon as the append returns. The
   *   cost of that shortcut is a real one: a concurrent `restoreFromSnapshot`
   *   that replaces this room's log is not waited out, so an appraisal appended
   *   just before the replace is erased with the rest of the log while the caller
   *   still holds the `appraisalId` it was given — a judgement the caller believes
   *   is recorded that the log no longer carries. Nothing else is corrupted; the
   *   statement is simply gone, which is the ordering rule the queue exists to
   *   prevent for state-backed facts.
   */
  ...
  async #recordAppraisal(room, sessionId, input) {
    ...
    if (about === sessionId) throw new Error("a member cannot appraise themselves");
    ...
    const known = await this.#knownEventIds(room.id);
    const unknown = evidenceEventIds.filter((id) => !known.has(id));
    if (unknown.length > 0) {
      throw new Error(`evidence event id not found in this room's log: ${unknown.join(", ")}`);
    }
    ...
    const written = await this.#record(room, event);
  ...
  async #revokeAppraisal(room, sessionId, input) {
    ...
    const target = revocableAppraisal(events, room.id, sessionId, about, tick);
    if (target === null) {
  ...
  /**
   * Every id this room's log carries that an agent can cite as evidence.
   *
   * One drained read, then set membership: an appraisal that cites several ids
   * costs no more lookups than one that cites a single id. The id domain is read
   * off the events rather than declared: the envelope's own `id`, plus the
   * payload identifiers the read-only surfaces hand to agents — a `messageId` and
   * a `ledger.transition` `entryId`. A charter proposal id is citable through
   * that `entryId`: proposing emits a `ledger.transition` whose `entryId` is the
   * proposal id, which `chat_memory` also hands back as the proposal's `id`.
   * There is deliberately no `payload.proposalId` case here — nothing emits a
   * `charter.*` event, so no event in this log carries that field, and scanning
   * for ids that cannot exist is how a reader is taught an id kind that cannot
   * be cited.
   */
  async #knownEventIds(roomId) {
    const events = await this.#readEvents(roomId);
    const known = new Set();
    for (const event of events) {
      if (typeof event?.id === "string" && event.id) known.add(event.id);
      const given = event?.payload ?? {};
      if (typeof given.messageId === "string" && given.messageId) known.add(given.messageId);
      if (typeof given.entryId === "string" && given.entryId) known.add(given.entryId);
    }
    return known;
  }
  ...
  async relationshipIntervention(roomId, input) {
    ...
    for (const key of Object.keys(input)) {
      if (!["action", "observerId", "targetId", "counters", "appliedBy", "mechanism", "note"].includes(key)) {
    ...
    const written = await this.#record(room, {
      ...
      payload: { action: input.action, observerId, targetId, counters, countersBefore,
        appliedBy: "human", mechanism, note },
    ...
    return { roomId: room.id, interventionId: written.id, tick: written.tick, action: input.action,
      observerId, targetId, counters, countersBefore, appliedBy: "human", mechanism, note };
  }
  ...
  async startRun(roomId, input) {
    ...
    const payload = { configHash: configHashOf(config), models: await this.#runModels(room),
  ...
  async #runModels(room) {
    ...
    for (const member of room.members) {
      ...
      models[member.sessionId] = { provider, model };
    }
    return models;
  }
  ...
  /**
   * Whether the room object an audit entry belongs to is still the live room.
   *
   * The discriminator is the object, not the room id. `restoreFromSnapshot`
   * installs a new object for the same room id, and only the object tells the
   * discarded room from the restored one: an entry queued against the object a
   * restore replaced fails here whenever it is asked — including when it was
   * re-queued by a failed write and re-claimed after the restore, which is the
   * hole a guard keyed on the room id leaves open.
   *
   * A room-less entry has no room to belong to; `#record` drops it, so it is
   * left to pass here and this check stays about the one question it owns.
   */
  #roomStillLive(room) {
    return this.state.rooms.find((item) => item.id === room?.id) === room;
  }

  /**
   * The log replace in flight for one entry's room, if any.
   *
   * The question is about the room id, not the room object: the object an entry
   * belongs to may be the one a restore detached, and only a replace of that
   * same id can bring it back, so only that same id can settle whether the entry
   * belongs to the room that survives. The registry holds at most one replace per
   * room — a restore's per-room lock keeps it from being two, and `open` refuses
   * a second — so this lookup is single-valued and the wait built on it cannot
   * face a choice between two gates.
   */
  #pendingLogReplaceFor(room) {
    return room ? this.pendingLogReplace.get(room.id) : null;
  }
  ...
  #queueAudit(room, build, stillValid, inSnapshot) {
    this.pendingAudit.push({ room, build, stillValid, ...(inSnapshot ? { inSnapshot } : {}) });
  }
  ...
  async #auditTurnSnapshot(room, rootMessageId) {
    ...
    try {
      ...
      if (runArm(events, room.id) === "reset_per_episode"
        ...
        if (reset !== null) events = [...events, reset];
      }
      derived = deriveRelationships({ events, roomId: room.id, asOfTick });
      appraisals = effectiveAppraisals(events, room.id, asOfTick);
      event = {
   ...
   *
   * Restores of one room id are serialised, so at most one log replace is in
   * flight for a room at any time. That is what keeps the registry's room-keyed
   * lookup single-valued, and with it the flush's wait: a flush can only ever
   * wait on the one replace a room has, never on a choice between two that
  ...
  async restoreFromSnapshot(snapshot, { confirm } = {}) {
    ...
    const roomId = checked.snapshot.room.id;
    const lock = this.#takeRestoreLock(roomId);
    try {
      await lock.previous;
      return await this.#restoreLocked(checked);
    } finally {
      lock.unlock();
    }
  }

  /** The restore body proper; runs holding the room's restore lock. */
  async #restoreLocked(checked) {
    ...
    // Publish the replace this restore owes, synchronously with the swap and
    // before anything is written, so a flush that could append to the restored
    // log waits for it instead of racing it. It is released once the replace is
    // registered on the log's own tail (or the restore has been abandoned),
    // because that registration is what makes an append land after the replace
    // rather than be erased by it. A flush also waits on it before refusing an
    // entry for the object this restore detached, so the refusal is decided
    // against a settled restore rather than a swap that may still be rolled back
    // below. `open` refuses a second replace for this room: the restore lock
    // means it never has one to refuse.
    let logReplace;
    try {
      logReplace = this.pendingLogReplace.open(restored.id);
    } catch (error) {
      this.state.rooms[index] = previous;
      throw error;
    }
    // The claim has to happen in the same synchronous step as the swap and the
    // publish above: an object created, installed and claimed together cannot
    // have anything else queue an audit against it in between.
    const saving = this.#beginSave();
    try {
      await saving.write;
    } catch (error) {
      // Nothing reached disk, so the room goes back and these audits wait for a
      // save that does. Releasing the replace here is what lets a flush that
      // held back for it re-ask the identity question against the rolled-back
      // state — where the room this entry belongs to is live again — instead of
      // dropping a fact that is the durable state.
      this.pendingAudit.unshift(...saving.claimed);
      this.state.rooms[index] = previous;
      this.pendingLogReplace.release(logReplace);
      throw error;
    }
    // The flush is started and published here, exactly where `#save` starts its
    // own — before the join on the write tail, so a reader of `auditFlush` sees
    // this restore's flush in the same window `#save` would have published it.
    // It will wait on this restore's replace, and that is the structural point of
    // the ordering below: the replace is registered and its entry released
    // *before* this restore joins the flush. Releasing a replace therefore never
    // waits for a flush, so the wait graph — flushes waiting on replaces — has no
    // edge out of a replace into a flush, and cannot contain a cycle. On the
    // round this replaces, the release came after joining the flush, and two
    // restores of one room could wait on each other's replace forever.
    const flushing = this.#flushAudits(saving.audited);
    ...
    // An audit entry belongs to a room object, and the log that object was
    // created against is the one now being replaced. With this restore settled,
    // the flush's identity check drops every entry whose object this restore
    // discarded — including one re-queued by a failed write and re-claimed
    // later, and including one it held back while the restore was still in
    // flight and could have been rolled back — so no such append can reach the
    // restored log. What is left is an entry claimed for the restored object,
    // which is a legitimate append that must survive: the release below is what
    // puts it after the replace instead of inside what the replace erases.
    let replaced;
    try {
      replaced = this.eventLog.replace(restored.id, checked.snapshot.events);
      this.pendingLogReplace.release(logReplace);
      await flushing;
      await replaced;
    } finally {
  ...
  /**
   * Take the restore lock for one room id and return it with its release.
   *
   * The chain is a promise that settles when the holder releases, so the next
   * restore of that room waits for the whole critical section — swap, state
   * write, log replace — and never overlaps it. Release is idempotent and drops
   * the tail only when it is still this holder's, so a queued restore cannot have
   * its own registration cleared by the one it waited for.
   */
  #takeRestoreLock(roomId) {
    const previous = this.restoreTails.get(roomId) ?? Promise.resolve();
    let release;
    const held = new Promise((resolve) => { release = resolve; });
    const tail = previous.then(() => held);
    this.restoreTails.set(roomId, tail);
    let released = false;
    return {
      previous,
      unlock: () => {
        ...
        released = true;
        release();
        if (this.restoreTails.get(roomId) === tail) this.restoreTails.delete(roomId);
      }
    };
  }
  ...
  #turnDigest(room, member, turnSample) {
    ...
    return relationshipDigest({ derived: turnSample?.derived, observer: member.sessionId,
      appraisals: this.appraisalDigest ? turnSample?.appraisals : undefined,
      labelOf: (sessionId) => aliases.get(sessionId) });
  }

  #participantPrompt(room, member, task, digest) {
  ...
  async #deliverOne(room, rootMessage, member, epoch, task, turnSample) {
    ...
    try {
      ...
      const digest = this.#turnDigest(room, member, turnSample);
      const promptText = this.#participantPrompt(room, member, task, digest);
