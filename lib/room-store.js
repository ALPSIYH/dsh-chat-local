import { NativeHostCompatibility, groupDeliveryMarkers } from "./host-compatibility.js";
...
import {AgentDirectory,migrateAgentDirectory,registerParticipations,visibleAppraisalEvents,observedEvidenceIds} from "./agent-directory.js";
import { AgentPersonas } from "./agent-persona.js";
import { ObservationCoverage } from "./observation-coverage.js";
import { AgentMemoryIndex } from "./agent-memory-index.js";
import { normalizeMemoryLifecycle, boundPersonalRecall } from "./memory-lifecycle.js";
import { StorageCapacity } from "./storage-capacity.js";
import { projectAgentMemory, renderPersonalMemory, personalRecallContext, nativeMemorySource, observedSessionItems, PERSONAL_MEMORY_VERSION } from "./agent-memory.js";
import { EventLog, verifyChain } from "./event-log.js";
import { RoomJournal } from "./room-journal.js";
import { deriveRelationships, latestRelationships, effectiveAppraisals, revocableAppraisal, revocableInterval,
  sampleRelationshipMemory, MEMORY_CONTRACT_VERSION,
  RELATIONSHIP_VERSION, APPRAISAL_EVENT_TYPE, APPRAISAL_STANCES, APPRAISAL_CLAIM_MAX_CHARS,
  APPRAISAL_CONFIDENCE_MIN, APPRAISAL_CONFIDENCE_MAX, RELATIONSHIP_COUNTER_NAMES,
  RELATIONSHIP_INTERVENTION_EVENT_TYPE, INTERVENTION_ACTIONS } from "./relationship.js";
import {
  // The digest, its terms and the hash of those terms all live in
  // `lib/experiment.js`: the injected text is rendered there from the same
  // constants and the same functions the hash covers, so no term and no
  // rendering rule can change the text without changing the hash. This module
  // only calls it, and re-exports the two names callers have always read here.
  RELATIONSHIP_DIGEST_MAX_CHARS, relationshipDigest, renderRelationshipDigest,
  injectionConfigFor, configHashOf, runArm, RUN_MANIFEST_EVENT_TYPE, RUN_ARMS,
  ARM_RESET_MECHANISM, episodeResetRecorded,
  INJECTION_COST_EVENT_TYPE, estimateTokens } from "./experiment.js";
import { classifyToolExecution, evaluateGate } from "./gate.js";
import { referencesPrivateState, isLocalRequestTarget } from "./restricted-read.js";
...
const RESTRICTED_READ_ALLOWED = new Set([
  ...
  "list_agents", "list_subagent_models", "get_goal", "team_task_list", "team_task_get", "wait_agent"
]);
/**
 ...
 * own relationship record could not ask for it. `chat_appraise` writes one
 * `appraisal` event into this room's own log and nothing else, which is the same
 * kind of room-state write as `chat_work` and `chat_charter_propose`: it changes
 * no user file, no permission and no other session.
 */
const GROUP_INTERNAL_TOOLS = new Set(["chat_send", "chat_rooms", "chat_memory", "chat_relationships", "chat_appraise", "chat_identity", "chat_recall", "chat_memory_update", "chat_charter_propose", "chat_charter_review", "chat_work", "chat_read_document", "chat_manage"]);
...
/**
 * An error a human-facing experiment call reports. `status` is the HTTP status
 * the route answers with: 400 for input this module will not guess at, 403 for a
 * call that is not the human's to make. Every refusal names its reason.
 */
function experimentError(message, status = 400) {
  return Object.assign(new Error(message), { status });
}

/**
 * One intervention's counter set, or `null` when it cannot be read. Only the
 * counters the caller states are kept — a `set` names what it changes and the
 * derivation reads the rest as zero — so the event records exactly what was
 * asked for. A name outside this version's vocabulary, a negative value, a value
 * that is not an integer, or an empty set makes the whole call invalid rather
 * than being rounded, dropped, or treated as a reset.
 */
function readInterventionCounters(value) {
  if (value === undefined || value === null) return null;
  if (typeof value !== "object" || Array.isArray(value)) return null;
  if (Object.keys(value).length === 0) return null;
  const counters = {};
  for (const [name, raw] of Object.entries(value)) {
    if (!RELATIONSHIP_COUNTER_NAMES.includes(name)) return null;
...
/** Transport sessions may vary across work, but one room has one seat per person. */
function assertDistinctRoomIdentities(room) {
  const identities = new Set();
  for (const member of room.members ?? []) {
    if (!member.agentId) continue;
    if (identities.has(member.agentId)) throw new Error(`duplicate Agent identity in room ${room.id}: ${member.agentId}; each person may occupy only one seat`);
    identities.add(member.agentId);
  }
}

/**
 * The digest, its terms and its cap live in `lib/experiment.js`, where
 * `injectionConfigFor` hashes them: the injected text, the hash that decides
 * whether two runs are comparable, and the rendering algorithm the hash now
 * covers are built from one set of constants and one set of functions. Both
 * names are re-exported here because this module is where callers have always
 * read the digest and its cap from.
 */
export { RELATIONSHIP_DIGEST_MAX_CHARS, relationshipDigest };
...
export class DshChatLocalService {
  constructor(ctx, config = {}) {
    this.ctx = ctx;
    this.hostCompatibility = new NativeHostCompatibility(ctx);
    this.path = config.path ?? join(homedir(), ".dsh", "dsh-chat-local", "rooms.json");
    // Committed state carries a durable outbox; startup recovers it before use.
    this.memoryLifecycle = normalizeMemoryLifecycle(config.memoryLifecycle);
    this.capacity = new StorageCapacity({statePath:this.path, ...config.storage});
    this.eventLog = new EventLog(this.path, {capacity:this.capacity});
    this.memoryIndex = new AgentMemoryIndex(this.memoryLifecycle);
    this.memoryRefreshTail = Promise.resolve();
    this.memoryMaintenanceAgents = new Set();
    this.memoryAuthoredAgents = new Set();
    this.storageMaintenanceOperations = new Set();
    this.memoryUnavailableSources = new Set();
    this.memoryMaintenanceTimer = null;
    this.memoryMaintenanceActive = null;
    this.memoryMaintenanceStatus = {runs:0,failures:0,lastError:null};
    this.journal = new RoomJournal({ path: this.path, events: this.eventLog, capacity:this.capacity,
      currentRoom: (roomId) => this.state.rooms.find((room) => room.id === roomId) });
    ...
    // Whether the judgement half of the relationship digest is injected at all.
    // It is a run-level setting, not a room's: it changes the text every member
    // reads, so it is a term of the run manifest's config hash (see
    // `injectionConfigFor`). Defaults to on, which is the behaviour every log
    // written before this setting existed was produced under.
    this.appraisalDigest = config.appraisalDigest !== false;
    this.personalMemory = config.personalMemory !== false;
    this.personas = new AgentPersonas(this.path, {capacity:this.capacity});
    this.observationCoverage = new ObservationCoverage();
    ...
    this.nativeContextSamples = new Map();
    this.observedToolCalls = new Map();
  ...
  async #load() {
    ...
    try {
      ...
      await this.eventLog.recoverAll();
      await this.journal.recover(parsed);
      ...
      for (const room of this.state.rooms) assertDistinctRoomIdentities(room);
      for (const agent of this.state.workspace.agents) await this.personas.read(agent);
      ...
      if (parsed.version !== STATE_VERSION) {
        // A file may legitimately carry no `version` at all (the load accepts
        // that), and interpolating it would name the backup `.vundefined.bak`.
        // The label says what the file is instead of borrowing a placeholder
        // from the language; a stated version keeps its own number so the
        // existing `.v15.bak` name and its regression test are unchanged.
        const from = Number.isInteger(parsed.version) ? parsed.version : "unversioned";
        try { await this.#backupState(`${this.path}.v${from}.bak`); }
        catch (backupError) { if (backupError?.code !== "ENOENT") throw backupError; }
      }
      // Recovery transitions are obligations of the state commit too: crashing
      // during recovery must not leave a failed delivery with no transition.
      for (const { room, message, delivery } of recoveredDeliveries) {
        const event = this.#deliveryEvent(message.id, delivery, "failed",
          { previous: delivery.previousStatus, error: delivery.error, recoveryReason: "restart" });
        this.#queueAudit(room, () => event, () => this.#deliveryPersisted(room, delivery));
      }
      if (parsed.version !== STATE_VERSION || legacyAgents.length > 0 || recovered || identitiesChanged) await this.#save();
    } catch (error) {
      if (error?.code !== "ENOENT" || stateFound) throw error;
    }
  }
  ...
  #normalizeRoom(input, legacyAgents) {
    ...
    room.roomSeq = room.messages.reduce((maximum, message) => Math.max(maximum, Number(message.roomSeq) || 0), Math.max(room.roomSeq, nextLegacySequence));
    ...
    assertDistinctRoomIdentities(room);
    return room;
  }

  /** Claim state and audits together; RoomJournal owns their durable ordering. */
  async #save() {
    migrateAgentDirectory(this.state);
    for (const room of this.state.rooms) assertDistinctRoomIdentities(room);
    const authored = [...this.memoryAuthoredAgents];
    this.memoryAuthoredAgents.clear();
    try {
      const result = await this.journal.commit(this.state);
      for (const id of authored) this.#scheduleMemory(id);
      return result;
    } catch (error) {
      for (const id of authored) this.memoryAuthoredAgents.add(id);
      throw error;
    }
  }
  ...
  async close() {
    this.closed = true;
    this.hostCompatibility.close();
    clearInterval(this.monitorTimer);
    if (this.memoryMaintenanceTimer) clearTimeout(this.memoryMaintenanceTimer);
    this.memoryMaintenanceTimer=null;
    await this.ready.catch(() => {});
    await this.personas.close();
    let changed = false;
    for (const capture of [...this.pending.values()]) {
      const room = this.state.rooms.find(item => item.id === capture.roomId);
      if (room?.epoch === capture.epoch) {
        this.#setDelivery(room, capture, "failed", { completedAt: Date.now(), error: "group-chat service closed" });
        changed = true;
      }
      this.#finishCapture(capture, { status: "failed", error: "group-chat service closed" });
    }
    this.pending.clear();
    ...
    if (this.memoryMaintenanceTimer) clearTimeout(this.memoryMaintenanceTimer);
    this.memoryMaintenanceTimer = null;
    this.memoryMaintenanceAgents.clear();
    await this.memoryMaintenanceActive?.catch(() => {});
    await this.memoryRefreshTail.catch(() => {});
    this.memoryIndex.close();
    await Promise.allSettled([...this.storageMaintenanceOperations]);
    await this.capacity.drain();
  }

  /**
   * Wait until the audit appends this service already owes have reached the log.
   *
   * An audit append is queued, not written: `#save` claims the queue with the
   * snapshot it is about to write and flushes it only after that write landed,
   * and the queue may hold entries no save has claimed yet. A reader that only
   * drained the log's own append chain can therefore report a history missing an
   * event that is already owed — the record exists in the writer, and the log
   * simply has not caught up. This seam joins both: the saves in flight, the
   * flush each of them performs, and the append chain underneath.
   *
   * It waits only on work already issued, so it never waits for a turn to end or
   * for a clock: a caller reaches the log's settled state without changing what
   * the room does next. Calling it is a read — it appends nothing of its own.
   */
  async settledAudit() {
    await this.ready;
    await this.journal.settled();
    return this.logHealth();
  }
  ...
  /** Public log reads are ordered with this instance's appends and replaces. */
  #readEvents(roomId) {
    return this.journal.readEvents(roomId);
  }

  /** Side-channel health, surfaced through /health. */
  logHealth() {
    return { ...this.eventLog.health(), journal: this.journal.health(), storage:this.capacity.health(),
      memory: { nativeContext: this.hostCompatibility.health(), coverage: this.observationCoverage.health(), index: this.memoryIndex.stats(), policy: this.memoryLifecycle,
        reads:{unavailableSourceCount:this.memoryUnavailableSources.size, scope:"last-verified-source-state-this-process"},
        maintenance:{...this.memoryMaintenanceStatus,queuedAgents:this.memoryMaintenanceAgents.size,active:!!this.memoryMaintenanceActive} } };
  }

  /** The state format the running service reads and writes. */
  stateVersion() { return STATE_VERSION; }

  #agentForSession(sessionId, roomId) {
    let agentId;
    if (roomId) agentId = this.#memoryMember(this.#room(roomId), sessionId).agentId;
    else {
      const identities = new Set(this.state.workspace.participations
        .filter(item => item.sessionId === sessionId && item.agentId).map(item => item.agentId));
      if (identities.size !== 1) throw new Error("session has no unambiguous Agent identity; use a room where you are a member");
      [agentId] = identities;
    }
    const agent = this.state.workspace.agents.find(item => item.id === agentId);
    if (!agent) throw new Error("Agent identity is unavailable");
    return agent;
  }

  #recallRoom(sessionId, requestedRoomId) {
    const lock = this.policyLocks.get(sessionId);
    if (!lock?.active) return requestedRoomId;
    if (lock.ambiguous || lock.stale) throw new Error("personal recall requires an unambiguous current participant turn");
    if (requestedRoomId && requestedRoomId !== lock.roomId) throw new Error("recall during an active group turn must use its current room");
    return lock.roomId;
  }

  /** An in-flight memory request cannot borrow a successor's identity or turn. */
  #memoryReadAuthority(sessionId, roomId, { requireTurn = false } = {}) {
    if (this.closed) throw new Error("memory service is closed");
    const room = roomId ? this.#room(roomId) : undefined;
    const member = room ? this.#memoryMember(room, sessionId, requireTurn) : undefined;
    const agent = this.#agentForSession(sessionId, roomId);
    const lock = this.policyLocks.get(sessionId), turn = this.turnBySession.get(sessionId);
    const validate = () => {
      if (this.closed) throw new Error("memory service is closed");
      if (room && this.state.rooms.find(item => item.id === room.id) !== room) {
        throw new Error("room was restored or replaced while accessing memory");
      }
      if (room && room.members.find(item => item.sessionId === sessionId) !== member) {
        throw new Error("participant identity changed while accessing memory");
      }
      if (this.policyLocks.get(sessionId) !== lock || this.turnBySession.get(sessionId) !== turn
        || this.#recallRoom(sessionId, roomId) !== roomId) throw new Error("participant turn changed while accessing memory");
      if (this.#agentForSession(sessionId, roomId).id !== agent.id) throw new Error("Agent identity changed while accessing memory");
      if (requireTurn) this.#memoryMember(room, sessionId, true);
    };
    return { agent, validate };
  }

  /** Personal identity is selected by the executing Session, never a caller-supplied Agent id. */
  async agentIdentity(sessionId, roomId) {
    await this.ready;
    roomId = this.#recallRoom(sessionId, roomId);
    const { agent, validate } = this.#memoryReadAuthority(sessionId, roomId);
    const { path, ...persona } = await this.personas.read(agent);
    validate();
    return { agentId: agent.id, alias: agent.alias, persona, memoryScope: "personally-observed", nativeContext: this.nativeContextStatus(sessionId) };
  }

  nativeContextStatus(sessionId) { return this.hostCompatibility.contextStatus(sessionId); }

  /** Inbox claims run synchronously before the Host assembles this turn's tools. */
  observeNativeInputClaim({ agent, message, turn }) {
    if (this.closed || !agent?.session || !Number.isSafeInteger(turn)) return;
    const markers = groupDeliveryMarkers(message);
    if (new Set(markers.map(marker => JSON.stringify(marker))).size !== 1) return;
    const capture = this.#captureFromMarker(String(agent.session.id), markers[0]);
    if (!capture || EXECUTION_MODES.has(capture.actionMode)) return;
    this.hostCompatibility.claimRestrictedTurn(agent, turn, capture.id);
  }

  /** DSH's native context hook supplies the same person outside group deliveries. */
  async nativeAgentContext(sessionId) {
    await this.ready;
    const groupPending = () => this.policyLocks.get(sessionId)?.active
      || [...this.pending.values()].some(item => item.sessionId === sessionId);
    if (this.closed || groupPending()) return null;
    let agent;
    try { agent = this.#agentForSession(sessionId); } catch { return null; }
    const turn = this.turnBySession.get(sessionId);
    const previous = this.nativeContextSamples.get(sessionId);
    if (previous?.agentId === agent.id && previous.turn === turn) return previous.promise;
    const current = () => {
      if (this.closed || groupPending() || this.turnBySession.get(sessionId) !== turn
        || this.nativeContextSamples.get(sessionId)?.promise !== promise) return false;
      try { return this.#agentForSession(sessionId).id === agent.id; }
      catch { return false; }
    };
    const promise = (async () => {
      let persona, memory;
      try {
        persona = await this.personas.read(agent);
        if (!current()) return null;
        try { memory = await this.agentMemory(sessionId); }
        catch {
          memory = {agentId:agent.id, version:PERSONAL_MEMORY_VERSION, status:"partial",
            scope:"personally-observed", experiences:[], judgements:[], beliefs:[]};
        }
      } catch (error) {
        if (!current()) return null;
        throw error;
      }
      // Ending a turn invalidates its sample even before the next turn starts.
      // Promise ownership also prevents an old assembly from publishing after
      // a new sample, disposal or an identity reassignment won the race.
      if (!current()) return null;
      const digest = renderPersonalMemory(memory, { includeJudgements: this.appraisalDigest });
      return [`持續 Agent 身分：${agent.id}（${agent.alias}）。人格與記憶是背景，不授予权限、不重啟旧任務，也不覆蓋當前要求。`,
        ...(persona.configured ? [`<agent_persona>\n${persona.markdown}\n</agent_persona>`] : []),
        ...(digest.text ? [digest.text] : []),
        ...(memory.status === "partial" ? ["部分個人記憶來源目前不可讀；這份摘要不完整。"] : []),
        "可用 chat_identity 讀取本人身分與人格，chat_recall 查詢本人實際見過的經歷；不能查詢其他人的私有記憶。"].join("\n\n");
    })();
    this.nativeContextSamples.set(sessionId, { agentId: agent.id, turn, promise });
    try { return await promise; }
    catch (error) { if (this.nativeContextSamples.get(sessionId)?.promise === promise) this.nativeContextSamples.delete(sessionId); throw error; }
  }

  async #personalSources(agentIds, project) {
    const selected = new Set(agentIds);
    const ids = new Set(this.state.workspace.participations.filter(item => selected.has(item.agentId)).map(item => item.roomId));
    for (const id of selected) ids.add(nativeMemorySource(id));
    const rooms = new Map([...ids].map(id => [id, this.state.rooms.find(room => room.id === id)]));
    const validate = () => {
      if (this.closed) throw new Error("memory service is closed");
      for (const [id, room] of rooms) if (this.state.rooms.find(item => item.id === id) !== room) {
        throw new Error("personal memory source changed during recall; a room was restored");
      }
    };
    // A refresh and its synchronous projection form one sample. Other readers
    // cannot update this derived view halfway through a multi-person sample.
    const operation = this.memoryRefreshTail.then(async () => {
      const unavailable = [], sourceRoomIds = [...ids].sort();
      for (const roomId of sourceRoomIds) {
        try {
          const view = await this.journal.readEventView(roomId, {after:this.memoryIndex.revision(roomId)});
          this.memoryIndex.syncSource(roomId, view);
          this.memoryUnavailableSources.delete(roomId);
        } catch {
          this.memoryUnavailableSources.add(roomId);
          this.memoryIndex.dropSource(roomId);
          unavailable.push(roomId);
        }
      }
      validate();
      const value = project({index:this.memoryIndex, sourceRoomIds, unavailable});
      return {value, unavailable, validate};
    });
    this.memoryRefreshTail = operation.then(() => {}, () => {});
    return operation;
  }

  #recallFromIndex({index, sourceRoomIds, unavailable}, options) {
    const projected = index.recall({...options, sourceRoomIds,
      consolidation:this.memoryLifecycle.consolidation, lifecycle:this.memoryLifecycle.decay,
      halfLifeMs:this.memoryLifecycle.halfLifeDays * 86400000, maxBytes:this.memoryLifecycle.maxRecallBytes});
    const {experiences, judgements, beliefs, ...base} = projected;
    return boundPersonalRecall({...base,
      status:unavailable.length || projected.coverage.partial ? "partial" : "available",
      unavailableSources:[...unavailable]}, [...experiences, ...judgements, ...(beliefs ?? [])],
      {limit:options.limit ?? 24, maxBytes:this.memoryLifecycle.maxRecallBytes});
  }

  /** All recall entry points share current authority, source indexing and policy. */
  async agentMemory(sessionId, { roomId, query = "", limit = 24, mode = "explicit" } = {}) {
    await this.ready;
    roomId = this.#recallRoom(sessionId, roomId);
    if (typeof query !== "string" || query.length > 2000) throw new Error("memory query must be at most 2000 characters");
    if (!Number.isInteger(limit) || limit < 1 || limit > 100) throw new Error("memory limit must be 1–100");
    const { agent, validate } = this.#memoryReadAuthority(sessionId, roomId);
    if (!this.personalMemory) return { agentId: agent.id, version: PERSONAL_MEMORY_VERSION,
      scope: "personally-observed", status: "disabled", experiences: [], judgements: [], beliefs: [] };
    const sample = await this.#personalSources([agent.id], view => {
      const context = personalRecallContext(roomId ? view.index.authorityContext(roomId) : undefined);
      return this.#recallFromIndex(view, {agentId:agent.id, query, limit, context, mode});
    });
    sample.validate(); validate();
    return sample.value;
  }

  /** Explicit self-management never changes raw evidence or personality. */
  async updatePersonalMemory(sessionId, input = {}) {
    await this.ready;
    if (!this.personalMemory) throw new Error("personal memory is disabled");
    if (!input || typeof input !== "object" || Array.isArray(input)) throw new Error("memory update must be an object");
    const allowed = ["roomId","action","operationId","sourceRoomId","evidenceId","claim","evidence","supersedes","beliefId"];
    for (const field of Object.keys(input)) if (!allowed.includes(field)) throw new Error(`unsupported memory update field: ${field}`);
    const {action, operationId} = input;
    if (!["pin","unpin","suppress","restore","belief","revoke_belief"].includes(action)) throw new Error("unsupported memory update action");
    if (typeof operationId !== "string" || !operationId.trim() || operationId.length > 200) throw new Error("memory operationId must be 1–200 characters");
    const roomId = this.#recallRoom(sessionId, input.roomId);
    const {agent, validate} = this.#memoryReadAuthority(sessionId, roomId, {requireTurn:!!this.policyLocks.get(sessionId)?.active});
    const sourceId = ["belief","revoke_belief"].includes(action) && roomId ? roomId : nativeMemorySource(agent.id);
    const operation = `personal-memory:${JSON.stringify([agent.id,operationId])}`;
    const writeRoom = sourceId === roomId ? this.#room(roomId) : null;
    const requestHash = createHash("sha256").update(JSON.stringify(Object.fromEntries(Object.entries(input).filter(([,v]) => v !== undefined).sort(([a],[b]) => a < b ? -1 : a > b ? 1 : 0)))).digest("hex");
    return this.journal.runRoom(sourceId, async () => {
      const prior = (await this.journal.readEvents(sourceId)).find(event => event.provenance?.operationId === operation);
      validate();
      if (prior) {
        if (prior.payload?.observerAgentId !== agent.id || prior.payload?.requestHash !== requestHash) throw new Error("memory operationId conflict");
        return {recorded:true, eventId:prior.id, operationId, beliefId:prior.payload.beliefId ?? null};
      }
      const sample = await this.#personalSources([agent.id], view => {
        const context = personalRecallContext(roomId ? view.index.authorityContext(roomId) : undefined);
        const lookup = (ref, includeSuppressed=false) => view.index.lookup({agentId:agent.id,
          sourceRoomIds:view.sourceRoomIds, context, ...ref, includeSuppressed});
        const payload = {observerAgentId:agent.id, operationId, requestHash};
        if (["pin","unpin","suppress","restore"].includes(action)) {
          if (typeof input.sourceRoomId !== "string" || typeof input.evidenceId !== "string") throw new Error("source and evidence id are required");
          const item=lookup({sourceRoomId:input.sourceRoomId,evidenceId:input.evidenceId},true);
          if (!item) throw new Error("memory control requires currently observed evidence");
          return {type:"memory.control",payload:{...payload,action,sourceRoomId:item.sourceRoomId,
            evidenceId:item.evidenceId,contentHash:item.contentHash,observationId:item.observationId ?? null}};
        }
        if (action === "revoke_belief") {
          if (typeof input.beliefId !== "string" || !view.index.belief({agentId:agent.id,beliefId:input.beliefId,sourceRoomIds:view.sourceRoomIds,context})) throw new Error("current own belief is required");
          return {type:"memory.belief",payload:{...payload,action:"revoke",beliefId:input.beliefId}};
        }
        if (typeof input.claim !== "string" || !input.claim.trim() || input.claim.length > 2000) throw new Error("belief claim must be 1–2000 characters");
        if (!Array.isArray(input.evidence) || !input.evidence.length || input.evidence.length > 8) throw new Error("belief requires 1–8 observed evidence references");
        const evidence=input.evidence.map(ref => {
          if (!ref || Object.keys(ref).some(k=>!["sourceRoomId","evidenceId"].includes(k))) throw new Error("invalid belief evidence reference");
          const item=lookup(ref); if(!item || item.kind === "judgement" || item.kind === "belief") throw new Error("belief requires original personally observed evidence");
          return {sourceRoomId:item.sourceRoomId,evidenceId:item.evidenceId,contentHash:item.contentHash,observationId:item.observationId ?? null};
        });
        if (input.supersedes !== undefined && (typeof input.supersedes !== "string" || !view.index.belief({agentId:agent.id,beliefId:input.supersedes,sourceRoomIds:view.sourceRoomIds,context}))) throw new Error("superseded belief must be your current belief");
        return {type:"memory.belief",payload:{...payload,action:input.supersedes ? "revise" : "record",
          beliefId:crypto.randomUUID(),claim:input.claim.trim(),evidence,supersedes:input.supersedes ?? null}};
      });
      sample.validate(); validate();
      const event={...sample.value,tick:writeRoom?.tick ?? 0,
        actor:{kind:"session",id:sessionId},provenance:{roomId:sourceId,actorId:sessionId,originClass:"agent",sessionKind:"interactive"}};
      const written=writeRoom ? await this.journal.appendChecked(writeRoom,event,validate,operation)
        : await this.eventLog.appendOnce(sourceId,event,operation);
      if (!written) throw new Error("personal memory update could not be persisted");
      validate();
      return {recorded:true,eventId:written.id,operationId,beliefId:written.payload.beliefId ?? null};
    });
  }

  /** Bind observations to the actual receiver; membership alone is not observation. */
  async #recordObservation(room, sessionId, payload) {
    const member = room.members.find(item => item.sessionId === sessionId);
    if (!member?.agentId) return null;
    const agentId = member.agentId;
    return this.observationCoverage.persist(() => this.journal.appendChecked(room, { type: "memory.observed", actor: { kind: "system", id: "system" },
      payload: { ...payload, version: PERSONAL_MEMORY_VERSION, observerAgentId: member.agentId, sessionId },
      provenance: { originClass: "system", sessionKind: "interactive" } }, () => {
        if (this.closed) throw new Error("memory service is closed");
        if (room.deletedAt || room.members.find(item => item.sessionId === sessionId)?.agentId !== agentId
          || (payload.observerAgentId && payload.observerAgentId !== agentId)) throw new Error("observation recipient identity changed");
      })).then(written => { this.#scheduleMemory(agentId); return written; });
  }

  #scheduleMemory(agentId) {
    if (this.closed || !this.personalMemory || !this.memoryLifecycle.consolidation) return;
    this.memoryMaintenanceAgents.add(agentId);
    if (this.memoryMaintenanceTimer || this.memoryMaintenanceActive) return;
    this.memoryMaintenanceTimer = setTimeout(() => {
      this.memoryMaintenanceTimer = null;
      if (this.closed) return;
      const batch=[...this.memoryMaintenanceAgents].slice(0,8);
      for (const id of batch) this.memoryMaintenanceAgents.delete(id);
      const running=this.#personalSources(batch, ({index,unavailable}) => {
        if (unavailable.length) throw new Error("memory background source is unavailable");
        return index.flush();
      });
      this.memoryMaintenanceActive=running;
      void running.then(() => {
        this.memoryMaintenanceStatus.runs++; this.memoryMaintenanceStatus.lastError=null;
      }, error => {
        this.memoryMaintenanceStatus.failures++;
        this.memoryMaintenanceStatus.lastError=String(error.code ?? "MEMORY_INDEX_UNAVAILABLE");
      }).finally(() => {
        this.memoryMaintenanceActive=null;
        const next=this.memoryMaintenanceAgents.values().next().value;
        if (next) this.#scheduleMemory(next);
      });
    },50);
    this.memoryMaintenanceTimer.unref?.();
  }

  /** Human maintenance preserves every referenced source; no Agent tool exposes it. */
  async maintainMemoryStorage({sourceRoomId,action} = {}) {
    await this.ready;
    if (this.closed) throw new Error("memory service is closed");
    const known=this.state.rooms.some(room=>room.id===sourceRoomId)
      || this.state.workspace.agents.some(agent=>nativeMemorySource(agent.id)===sourceRoomId);
    if (!known || !["archive","thaw","cleanup"].includes(action)) throw new Error("known memory source and maintenance action required");
    const operation = this.journal.runRoom(sourceRoomId, async () => {
      if (this.closed) throw new Error("memory service is closed");
      await this.journal.settled();
      if (this.closed) throw new Error("memory service is closed");
      const result=await (action==="archive" ? this.eventLog.archive(sourceRoomId)
        : action==="thaw" ? this.eventLog.thaw(sourceRoomId) : this.eventLog.cleanupArchives(sourceRoomId));
      if (!this.closed) this.memoryIndex.dropSource(sourceRoomId);
      return result;
    });
    this.storageMaintenanceOperations.add(operation);
    try { return await operation; }
    finally { this.storageMaintenanceOperations.delete(operation); }
  }

  /** Only native events actually emitted for a uniquely bound identity are retained. */
  async #observeNative(sessionId, event) {
    if (!["user/message", "assistant/message", "tool/result"].includes(event?.type)) return;
    if (this.closed || !this.personalMemory) return;
    const lock = this.policyLocks.get(sessionId);
    const groupPending = [...this.pending.values()].some(item => item.sessionId === sessionId);
    if (lock?.active || groupPending) {
      // Group output is captured by observeSessionEvent's delivery branch.
      // Other user messages (including native Team peer messages) still reach
      // this same person while a group delivery is queued or running.
      if (lock?.active) {
        const items = this.#sessionObservationItems(sessionId, event);
        if (!items.length) return;
        const capture = this.#captureFromMarker(sessionId, { deliveryId: lock.captureId, roomId: lock.roomId });
        if (event.type !== "user/message" || lock.ambiguous || lock.stale || !capture || !capture.observerAgentId
          || capture.turn !== lock.turn || capture.turn !== this.turnBySession.get(sessionId)) {
          this.observationCoverage.count("ambiguousIdentityEvents");
          return;
        }
        // The checked room append retains the actual recipient identity and
        // the current episode's reset scope; it never imports the sender's
        // private history or grants other room members an observation.
        await this.#recordObservation(this.#room(capture.roomId), sessionId, {
          origin: "native-message", deliveryId: capture.id, observerAgentId: capture.observerAgentId, items
        });
        return;
      }
      // A queued prompt is not evidence that its recipient has consumed it.
      // Before its marker, only the Session's unambiguous native identity may
      // own incoming material, using its native source rather than that room.
      // Its output must also name the current observed turn: an old result
      // arriving after turn/end or the next turn must not borrow that identity.
      if (event.type !== "user/message" && (event.data?.turn === undefined
        || event.data.turn !== this.turnBySession.get(sessionId))) {
        this.observationCoverage.count("ambiguousIdentityEvents");
        return;
      }
    }
    let agent;
    try { agent = this.#agentForSession(sessionId); } catch {
      const identities = new Set(this.state.workspace.participations.filter(item => item.sessionId === sessionId).map(item => item.agentId));
      this.observationCoverage.count(identities.size > 1 ? "ambiguousIdentityEvents" : "unboundEvents");
      return;
    }
    const items = this.#sessionObservationItems(sessionId, event);
    if (!items.length) return;
    const roomId = nativeMemorySource(agent.id);
    await this.observationCoverage.persist(() => this.eventLog.append(roomId, { type: "memory.observed", tick: 0,
      actor: { kind: "session", id: sessionId }, payload: { version: PERSONAL_MEMORY_VERSION,
        observerAgentId: agent.id, sessionId, origin: "native-session", items },
      provenance: { roomId, actorId: sessionId, originClass: "system", sessionKind: "interactive" } }));
    this.#scheduleMemory(agent.id);
  }

  #sessionObservationItems(sessionId, event) {
    return observedSessionItems(event).map(item => ({ ...item,
      id: `session:${JSON.stringify([sessionId, item.id ?? item.sourceSeq ?? crypto.randomUUID()])}` }));
  }
   ...
   *
   * It is the C backbone only, and deliberately not a merge with the A layer: a
   * snapshot states counters and never a judgement, so folding an appraisal in
   * here would hand this projection's own readers — the governance gate among
   * them — a number no event ever stated. Appraisals have their own projection
   * in `appraisals` below, and the injected prompt is the one place the two are
   * presented together: there, and only there, with the judgement labelled as a
   * judgement.
   *
  ...
  /**
   * The appraisals currently in force in one room, keyed by observer and then by
   * target: each observer's own statements about other members, never another
   * observer's.
   *
   * This is the A overlay's own read path, kept separate from `relationships`
   * on purpose. The C projection is what the room's `relationship.snapshot`
   * events stated and is the same under every observer; an appraisal is one
   * member's judgement, carries no counter and never enters a snapshot. Merging
   * the two into one object would make a judgement indistinguishable from a
   * count on the wire, which is exactly the confusion the layer exists to avoid.
   *
   * The horizon is the room's current tick, so a statement is current at the
   * moment it is made and a revocation takes effect at the tick it states. A log
   * that cannot be read degrades to `{}`, the same honest answer `relationships`
   * gives: "no appraisal state is readable here".
   */
  async appraisals(roomId) {
    await this.ready;
    const room = this.#room(roomId);
    let events;
    try { events = await this.#readEvents(room.id); }
    catch { return {}; }
    return effectiveAppraisals(visibleAppraisalEvents(events, { roomId: room.id, members: room.members,
      participations: this.state.workspace.participations }), room.id, room.tick ?? 0);
  }
   ...
   *
   * The counters are indexed out of `relationships`, never derived a second
   * time, so a caller that stubs or overrides the projection sees that
   * projection here too. `appraisals` carries only this observer's own
   * judgements about others — the one part of the row that is genuinely theirs —
   * under its own key, so a reader can tell a count from a judgement without
   * inspecting the text. A self-appraisal is refused at the writer, so the key
   * set is "this observer's statements about their counterparties"; the
   * observer's own key is left out rather than projected as an appraisal of
   * themselves.
   */
  async relationshipRow(roomId, sessionId) {
    await this.ready;
    const room = this.#room(roomId);
    const { validate } = this.#memoryReadAuthority(sessionId, roomId);
    const targets = (await this.relationships(room.id))[sessionId] ?? {};
    let appraisals;
    try { appraisals = effectiveAppraisals(visibleAppraisalEvents(await this.#readEvents(room.id), {
      roomId: room.id, members: room.members, participations: this.state.workspace.participations }), room.id, room.tick ?? 0); }
    catch { appraisals = {}; }
    validate();
    return { observer: sessionId, targets, appraisals: appraisals[sessionId] ?? {} };
  }

  /**
   * Record, or revoke, one member's appraisal of another.
   *
   * This is the A layer's only writer, and it is deliberately narrow — the plan
   * leaves four decisions open and they are settled here:
   *
   * - **Evidence is mandatory and personally observed.** `evidenceEventIds`
   *   is a non-empty list checked against one ordered read of this room's log.
   *   Confirmed deliveries and explicit memory reads record which material this
   *   stable Agent identity received. Observed messages/ledger items and their
   *   envelopes, the Agent's own authored messages, and its own appraisal ids
   *   are citable. Merely knowing another observer's private appraisal id, or
   *   belonging to a room containing an unread message, grants no evidence.
   *   Unknown/unobserved ids are refused as a whole; silently dropping one would
   *   record a different statement from the one the caller made.
   * - **Only room members, and never oneself.** Both are refused, because an
   *   appraisal of a non-member would be a judgement about someone who is not in
   *   the room, and a self-appraisal would let the observer axis carry a pair
   *   that is not one member seeing another.
   * - **Revocation, not deletion.** `revoke` appends one more `appraisal` event
   *   that repeats the statement it retires and sets `validTo` to the current
   *   tick. The record it revoked stays in the log, byte for byte, and the
   *   projection stops selecting it because it is outside its own interval. An
   *   already-revoked statement is refused rather than double-revoked, and a
   *   revocation that would state an empty interval is refused because it would
   *   claim to have retired something it never covered.
   * - **The commit boundary.** The journal waits for any in-flight replacement,
   *   then checks room identity and active-turn authority before registering the
   *   append. An appraisal whose evidence read outlived a restore is refused.
   *   Successful prior appraisals remain subject to a later explicit restore.
   */
  async appraise(roomId, sessionId, input) {
    await this.ready;
    const room = this.#room(roomId);
    // `true` ties an appraisal to this room's active, non-superseded turn, in
    // step with every other room-state write an agent may make.
    const { validate } = this.#memoryReadAuthority(sessionId, roomId, { requireTurn: true });
    const allowed = new Set(["action", "aboutAgentId", "stance", "confidence", "claim", "perceivedRole", "evidenceEventIds", "appraisalId"]);
    if (!input || typeof input !== "object" || Array.isArray(input)) throw new Error("appraisal input must be an object");
    for (const key of Object.keys(input)) if (!allowed.has(key)) throw new Error(`unsupported appraisal field: ${key}`);
    const action = input.action ?? "record";
    if (!["record", "revoke"].includes(action)) throw new Error("appraisal action must be record or revoke");
    return action === "revoke"
      ? this.#revokeAppraisal(room, sessionId, input, validate)
      : this.#recordAppraisal(room, sessionId, input, validate);
  }

  /** Append one new appraisal statement. See `appraise` for the settled rules. */
  async #recordAppraisal(room, sessionId, input, validate) {
    const about = ensureText(input.aboutAgentId, "appraisal target");
    if (about === sessionId || room.members.find(member => member.sessionId === about)?.agentId
      === room.members.find(member => member.sessionId === sessionId)?.agentId) throw new Error("a member cannot appraise themselves");
    if (!room.members.some((member) => member.sessionId === about)) {
      throw new Error("appraisal target must be a member of this room");
    }
    if (!APPRAISAL_STANCES.includes(input.stance)) {
      throw new Error(`stance must be one of ${APPRAISAL_STANCES.join(", ")}`);
    }
    const confidence = input.confidence;
    if (typeof confidence !== "number" || !Number.isFinite(confidence)
      || confidence < APPRAISAL_CONFIDENCE_MIN || confidence > APPRAISAL_CONFIDENCE_MAX) {
      throw new Error(`confidence must be a number in [${APPRAISAL_CONFIDENCE_MIN}, ${APPRAISAL_CONFIDENCE_MAX}]`);
    }
    const claim = ensureText(input.claim, "appraisal claim");
    if (claim.length > APPRAISAL_CLAIM_MAX_CHARS) throw new Error(`appraisal claim exceeds ${APPRAISAL_CLAIM_MAX_CHARS} characters`);
    const perceivedRole = optionalText(input.perceivedRole, "perceived role", APPRAISAL_CLAIM_MAX_CHARS);
    if (!Array.isArray(input.evidenceEventIds) || input.evidenceEventIds.length === 0) {
      throw new Error("an appraisal requires at least one evidence event id");
    }
    const evidenceEventIds = [...new Set(input.evidenceEventIds.map((id) => ensureText(id, "evidence event id")))];
    const known = await this.#knownEventIds(room, sessionId);
    validate();
    const unknown = evidenceEventIds.filter((id) => !known.has(id));
    if (unknown.length > 0) {
      throw new Error(`evidence event id not found in this room's log or not observed by this Agent: ${unknown.join(", ")}`);
    }
    const tick = room.tick ?? 0;
    const event = {
      type: APPRAISAL_EVENT_TYPE,
      actor: { kind: "session", id: sessionId },
      payload: {
        observerId: sessionId,
        aboutAgentId: about,
        observerAgentId: room.members.find((member) => member.sessionId === sessionId)?.agentId ?? null,
        targetAgentId: room.members.find((member) => member.sessionId === about)?.agentId ?? null,
        stance: input.stance,
        confidence,
        claim,
        perceivedRole: perceivedRole ?? null,
        evidenceEventIds,
        validFrom: tick,
        validTo: null,
        action: "record"
    ...
    const record = { roomId: room.id, observerId: sessionId, aboutAgentId: about, stance: input.stance,
      confidence, claim, perceivedRole: perceivedRole ?? null, evidenceEventIds, validFrom: tick, validTo: null };
    const written = await this.journal.appendChecked(room, event, validate);
    if (written === null) throw new Error("this room's log is unavailable; the appraisal was not recorded");
    return { ...record, appraisalId: written.id };
  }

  /**
   * Retire one appraisal by appending the statement that closes its interval.
   *
   * The record itself is copied forward rather than mutated or removed: the log
   * is append-only, so "revoked" is a fact a later event states, never a
   * rewrite. The revocation names the statement by the id of the event that
   * carried it, so the caller cannot revoke a pair by guessing and a statement
   * that a newer one replaced is not what gets retired.
   */
  async #revokeAppraisal(room, sessionId, input, validate) {
    const about = ensureText(input.aboutAgentId, "appraisal target");
    const tick = room.tick ?? 0;
    const events = await this.#readEvents(room.id);
    validate();
    const target = revocableAppraisal(visibleAppraisalEvents(events, { roomId: room.id, members: room.members,
      participations: this.state.workspace.participations }), room.id, sessionId, about, tick);
    if (target === null) {
      throw new Error("no effective appraisal of that member is on record; nothing to revoke");
    }
    if (input.appraisalId !== undefined && ensureText(input.appraisalId, "appraisal id") !== String(target.id)) {
      throw new Error("the named appraisal is not the effective one; read chat_relationships and revoke that statement");
    }
    if (!revocableInterval(target, tick)) {
      throw new Error("this turn shares the tick of the appraisal it would revoke; revoke it in a later turn");
    }
    const given = target.payload ?? {};
    const event = {
      type: APPRAISAL_EVENT_TYPE,
      actor: { kind: "session", id: sessionId },
      payload: {
        ...
        claim: given.claim,
        perceivedRole: given.perceivedRole ?? null,
        evidenceEventIds: [...(given.evidenceEventIds ?? [])],
        validFrom: Number.isFinite(given.validFrom) ? given.validFrom : tick,
        validTo: tick,
        action: "revoke",
        revokesAppraisalId: String(target.id)
      },
      provenance: { originClass: "agent", sessionKind: "interactive" }
    };
    const written = await this.journal.appendChecked(room, event, validate);
    if (written === null) throw new Error("this room's log is unavailable; the appraisal was not revoked");
    return { roomId: room.id, observerId: sessionId, aboutAgentId: about, revokedAppraisalId: String(target.id),
      appraisalId: written.id, validFrom: event.payload.validFrom, validTo: tick };
  }

  /** Resolve only this identity's observed or authored evidence, within this room. */
  async #knownEventIds(room, sessionId) {
    const agentId = room.members.find((member) => member.sessionId === sessionId)?.agentId;
    const events = await this.#readEvents(room.id);
    const identities = new Set(this.state.workspace.participations
      .filter((item) => item.roomId === room.id && item.sessionId === sessionId && item.agentId)
      .map((item) => item.agentId));
    return observedEvidenceIds(events, { roomId: room.id, sessionId, agentId,
      unambiguousSession: identities.size === 1 && identities.has(agentId) });
  }

  /**
   * Apply, or seed, a human intervention in one room's relationship counters.
   *
   * This is the experiment's only lever: it records a
   * `relationship.intervention` event, and `lib/relationship.js` reads that
   * event as the start of a new counting window for the targets it names. It
   * never rewrites an existing event, and a `clear` resets rather than deletes —
   * the whole history stays in the log, which is what makes a reset arm
   * reviewable after the fact.
   *
   * **Human-only, and closed to a running group turn.** This endpoint is
   * deliberately not an agent tool: an agent that could clear its own counters
   * would be rewriting both the experiment's independent variable (what the next
   * prompt injects) and its dependent variables (the counters the gate and the
   * evaluation read), and no later analysis could repair that. `appliedBy` must
   * be exactly `"human"`, and the call is refused while any member of this room
   * holds an active group-chat turn — a call from inside a turn is therefore
   * refused by construction, not by convention. Refusing during a turn is also
   * what keeps the record unambiguous: an intervention mid-turn could land
   * between a turn's snapshot and its prompt, so the counters a member read
   * would not be the counters the log states for that turn.
   *
   * **The counters before are recorded.** `countersBefore` is the derivation as
   * of the room's current tick, read before the event is appended, keyed by
   * target: without it "what was reset" cannot be reviewed. A counter describes
   * its target and is identical under every observer, so a per-target map is the
   * complete answer for the scope; `observerId` is recorded for the caller's
   * benefit and does not narrow the effect (see `interventionWindows`).
   *
   * **Invalid input is refused with a reason**, never silently ignored: an
   * unknown action, a missing mechanism, a session that is not a member of this
   * room, a counter name this version does not define, a counter value that is
   * not a non-negative integer, and a `clear` that carries counters are all
   * errors. `set`/`seed` may name only the counters they change; the rest start
   * at zero.
   */
  async relationshipIntervention(roomId, input) {
    await this.ready;
    const room = this.#room(roomId);
    this.#assertHumanExperimentCall(room, input, "relationship intervention");
    for (const key of Object.keys(input)) {
      if (!["action", "observerId", "targetId", "counters", "appliedBy", "mechanism", "note", "memoryScope"].includes(key)) {
        throw experimentError(`unsupported intervention field: ${key}`);
      }
    }
    if (!INTERVENTION_ACTIONS.includes(input.action)) {
      throw experimentError(`action must be one of ${INTERVENTION_ACTIONS.join(", ")}`);
    }
    const mechanism = optionalText(input.mechanism, "intervention mechanism", 200);
    if (!mechanism) throw experimentError("an intervention requires a mechanism naming how it was applied");
    const note = input.note === undefined || input.note === null
      ? null : optionalText(input.note, "intervention note", 2_000);
    const observerId = this.#interventionMember(room, input.observerId, "observerId");
    const targetId = this.#interventionMember(room, input.targetId, "targetId");
    const memoryScope = input.memoryScope ?? "counters";
    if (!["counters", "all"].includes(memoryScope) || (memoryScope === "all" && input.action !== "clear")) {
      throw experimentError("memoryScope must be counters, or all with action clear");
    }
    if (memoryScope === "all" && (observerId !== null || targetId !== null)) {
      throw experimentError("clearing all memory layers requires a whole-room scope; use counters for a scoped intervention");
    }
    let counters = null;
    if (input.action === "clear") {
      if (input.counters !== undefined && input.counters !== null) {
        throw experimentError("clear takes no counters; it resets the scope it names to zero");
      }
    } else {
      counters = readInterventionCounters(input.counters);
      if (counters === null) {
        throw experimentError(`counters must be an object whose values are non-negative integers named from: ${RELATIONSHIP_COUNTER_NAMES.join(", ")}`);
      }
    }
    // One drained read, before the append: the "before" the audit exists for
    // must not be able to include the intervention itself.
    const events = await this.#readEvents(room.id);
    const before = deriveRelationships({ events, roomId: room.id, asOfTick: room.tick ?? 0 });
    const scoped = targetId === null ? room.members.map((member) => member.sessionId) : [targetId];
    const countersBefore = {};
    for (const target of scoped) {
      // A counter is the target's own record, so the target's self-row is the
      // complete "before" under every observer.
      const row = before.pairs.find((pair) => pair.observer === target && pair.target === target);
      countersBefore[target] = { ...(row?.counters ?? {}) };
    }
    const written = await this.journal.appendChecked(room, {
      type: RELATIONSHIP_INTERVENTION_EVENT_TYPE,
      actor: { kind: "human", id: "human:me" },
      payload: { action: input.action, observerId, targetId, counters, countersBefore,
        appliedBy: "human", mechanism, note, memoryScope, memoryVersion: MEMORY_CONTRACT_VERSION },
      provenance: { originClass: "owner", sessionKind: "interactive" }
    }, () => this.#assertHumanExperimentCall(room, input, "relationship intervention"));
    if (written === null) throw experimentError("this room's log is unavailable; the intervention was not recorded", 503);
    return { roomId: room.id, interventionId: written.id, tick: written.tick, action: input.action,
      observerId, targetId, counters, countersBefore, appliedBy: "human", mechanism, note, memoryScope, memoryVersion: MEMORY_CONTRACT_VERSION };
  }

  /**
   * Open a run on one room: record the configuration the run is judged by.
   *
   * The manifest is an event, and it is the **only** statement of a run's fixed
   * configuration: `arm` (whether relationship state persists across episodes),
   * the models actually in use, the state format, the tick the run started at,
   * and the hash of every term that affects what the relationship digest
   * injects.
   *
   * `models` is read from the runtime, never from the caller. This harness
   * cannot be told which model a member runs (the `subagent` tool takes no model
   * argument), so a run records what the sessions report and `null` where the
   * runtime cannot report it — a caller-supplied model name would be a fiction a
   * later reader could not detect. `appliedBy` must be exactly `"human"` and the
   * call is refused during an active group turn for the same reasons the
   * intervention is: the arm is the independent variable.
   *
   * The arm recorded here governs this room from now on: the turn path reads it
   * back out of the log (`runArm`), so a restart cannot silently change arms,
   * and two rooms in one process may hold different arms.
   */
  async startRun(roomId, input) {
    await this.ready;
    const room = this.#room(roomId);
    const configurationSequence = room.roomSeq;
    this.#assertHumanExperimentCall(room, input, "run manifest");
    for (const key of Object.keys(input)) {
      if (!["arm", "appliedBy"].includes(key)) throw experimentError(`unsupported run manifest field: ${key}`);
    }
    if (!RUN_ARMS.includes(input.arm)) throw experimentError(`arm must be one of ${RUN_ARMS.join(", ")}`);
    const personas = {};
    for (const member of room.members) {
      const agent = this.state.workspace.agents.find(item => item.id === member.agentId);
      if (agent) personas[agent.id] = (await this.personas.read(agent)).hash;
    }
    const config = injectionConfigFor({ room, appraisalDigest: this.appraisalDigest,
      personalMemory: this.personalMemory, memoryLifecycle:this.memoryLifecycle, personas });
    const payload = { configHash: configHashOf(config), config, models: await this.#runModels(room),
      initialStateVersion: STATE_VERSION, startedAtTick: room.tick ?? 0, arm: input.arm };
    const written = await this.journal.appendChecked(room, {
      type: RUN_MANIFEST_EVENT_TYPE,
      actor: { kind: "human", id: "human:me" },
      payload,
      provenance: { originClass: "owner", sessionKind: "interactive" }
    }, () => {
      this.#assertHumanExperimentCall(room, input, "run manifest");
      if (room.roomSeq !== configurationSequence) throw experimentError("room configuration changed while preparing its run manifest; retry", 409);
    });
    if (written === null) throw experimentError("this room's log is unavailable; the run manifest was not recorded", 503);
    return { roomId: room.id, manifestId: written.id, tick: written.tick, ...payload, config };
  }

  /**
   * The real provider/model of each member, as the runtime reports it.
   *
   * A member whose configuration cannot be read is recorded as `null`, not
   * guessed and not dropped: an evaluation must be able to see that a run's
   * models are unknown rather than compare it as though they were known.
   */
  async #runModels(room) {
    const models = {};
    for (const member of room.members) {
      let provider = null;
      let model = null;
      let reasoningEffort;
      try {
        const config = await snapshotMemberConfiguration(this.ctx, { sessionId: member.sessionId, alias: member.alias });
        if (typeof config?.model?.provider === "string" && config.model.provider) provider = config.model.provider;
        if (typeof config?.model?.model === "string" && config.model.model) model = config.model.model;
        if (typeof config?.model?.reasoningEffort === "string") reasoningEffort = config.model.reasoningEffort;
      } catch { /* left null: the runtime could not report it, and a guess is worse */ }
      models[member.sessionId] = { provider, model, ...(reasoningEffort === undefined ? {} : { reasoningEffort }) };
    }
    return models;
  }

  /**
   * The human-only gate every experiment mutation passes through.
   *
   * `appliedBy` is the caller's explicit claim and must be exactly `"human"`;
   * anything else — including an absent field — is refused as unauthorized.
   * The active-turn check is the structural half: while any member of this room
   * is inside a group-chat turn, no experiment mutation is accepted, so a call
   * made from within a turn cannot land however it reached this method.
   */
  #assertHumanExperimentCall(room, input, label) {
    if (!input || typeof input !== "object" || Array.isArray(input)) {
      throw experimentError(`${label} input must be an object`);
    }
    if (input.appliedBy !== "human") {
      throw experimentError(`${label} may only be applied by a human; appliedBy must be "human"`, 403);
    }
    if (this.#groupTurnActive(room.id)) {
      throw experimentError(`${label} refused: a group-chat turn is active in this room. An agent's turn never changes the experiment's own variables; wait for the room to go idle and intervene as the user.`, 403);
    }
  }

  /** Whether any member of this room holds an active group-chat turn right now. */
  #groupTurnActive(roomId) {
    const room = this.state.rooms.find(item => item.id === roomId);
    if (["running", "queued"].includes(room?.orchestration?.state)
      || [...this.pending.values()].some(item => item.roomId === roomId && !item.settled)) return true;
    for (const lock of this.policyLocks.values()) {
      // `lock !== undefined` first: the optional comparison alone is vacuously
      // true for an absent lock and an absent `roomId`, and `lock.active` then
      // dereferences nothing. The only caller passes a room's own id, so this is
      // a shape fix rather than a live crash.
      if (lock !== undefined && lock.roomId === roomId && lock.active === true) return true;
    }
    return false;
  }

  /**
   * One optional intervention scope field, resolved against this room's members.
   * A blank or absent value is `null` ("no scope stated"); a value that names
   * someone this room does not hold is refused rather than silently dropped.
   */
  #interventionMember(room, value, field) {
    if (value === undefined || value === null || value === "") return null;
    const sessionId = ensureText(value, `intervention ${field}`);
    if (!room.members.some((member) => member.sessionId === sessionId)) {
      throw experimentError(`intervention ${field} is not a member of this room: ${sessionId}`);
    }
    return sessionId;
  }
  ...
  /** Observational events are best-effort; EventLog counts failures. */
  #record(room, event) {
    return this.journal.record(room, event);
  }
   ...
   * write re-queues every entry and appends nothing.
   *
   * `inSnapshot` is decided at claim time, against the state the claim is about
   * to serialise, for facts that live objects carry and mutate. `stillValid` is
   * re-checked at flush time and against the state that save wrote: an entry
   * whose state did not survive is dropped, never appended. An entry with no
   * `inSnapshot` is one whose fact cannot move under a snapshot, so the claim
   * alone covers it. Independently of both, and for every entry, the flush also
   * asks whether the room object this fact belongs to is still the live one and
   * whether that room's log is mid-replace. A future event type that forgets to
   * restate the room in its `stillValid` is still covered.
   */
  #queueAudit(room, build, stillValid, inSnapshot) {
    this.journal.queue(room, build, stillValid, inSnapshot);
  }
  ...
  #auditMessage(room, message) {
    ...
    const agentId = message.authorKind === "session" && room.members.find(item => item.sessionId === message.author)?.agentId;
    if (agentId) this.memoryAuthoredAgents.add(agentId);
    if (agentId) this.#queueAudit(room, () => ({ type: "memory.observed", actor: { kind: "system", id: "system" },
      payload: { version: PERSONAL_MEMORY_VERSION, observerAgentId: agentId, sessionId: message.author,
        origin: "authored", messageIds: [message.id] }, provenance: { originClass: "system", sessionKind: "interactive" } }),
    () => room.messages.some(item => item.id === message.id)
      && room.members.some(item => item.sessionId === message.author && item.agentId === agentId));
  }
   ...
   *
   * The same read and the same tick also produce the turn's appraisals, and they
   * travel to the prompt together with the counters. Appraisals are read here
   * rather than in `#participantPrompt` for the reason the counters are: one read
   * per turn, one horizon, so two members of one turn cannot be told different
   * things about the same statement. They are never folded into the derivation:
   * an appraisal is a judgement and no counter is derived from one.
   */
  async #auditTurnSnapshot(room, rootMessageId) {
    let event, derived, appraisals, personal = {}, personas = {}, personaHashes = {}, configHash;
    const samplePersonas = async () => {
      for (const member of room.members) {
        const agent = this.state.workspace.agents.find(item => item.id === member.agentId);
        if (agent) {
          const persona = await this.personas.read(agent);
          personas[member.sessionId] = persona;
          personaHashes[agent.id] = persona.hash;
        }
      }
    };
    try {
      let events = await this.#readEvents(room.id);
      const asOfTick = room.tick ?? 0;
      // The arm is read out of this room's own log rather than from a process
      // setting: a run's arm is the experiment's independent variable, the log
      // is where it is stated, and a restart must not be able to change it
      // silently. Under `reset_per_episode` this episode's reset is appended
      // here — after the turn's tick is durable and before the derivation — so
      // the snapshot and every prompt of this episode read the reset counters,
      // and the log carries what the reset closed over. It is appended once per
      // episode: a retry of a failed delivery runs another `#runTurn` for the
      // same root message, which is the same episode, not a second one.
      if (runArm(events, room.id) === "reset_per_episode"
        && !episodeResetRecorded(events, room.id, rootMessageId)) {
        const reset = await this.#appendEpisodeReset(room, events, asOfTick, rootMessageId);
        if (reset === null) throw new Error("episode memory reset could not be recorded");
        events = [...events, reset];
      }
      ({ derived, appraisals } = sampleRelationshipMemory({ events: visibleAppraisalEvents(events,
        { roomId: room.id, members: room.members, participations: this.state.workspace.participations }), roomId: room.id, asOfTick }));
      const actorIds = room.members.map(member => member.agentId).filter(Boolean);
      const arm = runArm(events, room.id);
      const context = personalRecallContext({ roomId: room.id, events, arm });
      const query = room.messages.find(message => message.id === rootMessageId)?.text ?? "";
      if (this.personalMemory) try {
        const sample = await this.#personalSources(actorIds, view => Object.fromEntries(room.members.filter(member=>member.agentId).map(member =>
          [member.sessionId,this.#recallFromIndex(view,{agentId:member.agentId,query,
            targetAgentIds:actorIds.filter(id=>id!==member.agentId),context,mode:"default",
            sourceOverrides:[{roomId:room.id,events}]} )])));
        sample.validate(); personal=sample.value;
      } catch {
        // Unavailable personal policy fails closed without erasing the independent persona.
        personal = {};
      }
      await samplePersonas();
      configHash = configHashOf(injectionConfigFor({ room, appraisalDigest: this.appraisalDigest,
        personalMemory: this.personalMemory, memoryLifecycle:this.memoryLifecycle, personas: personaHashes }));
      event = {
        type: "relationship.snapshot",
        actor: { kind: "system", id: "system" },
        payload: {
          // Copied per pair, never handed out by reference: `derived.pairs` is
          // the live value this turn is about to read, and a snapshot payload
          // that aliased it would let any later consumer that amended what it
          // was given rewrite the event that is already queued.
          pairs: derived.pairs.map(({ observer, target, tick, counters }) =>
            ({ observer, target, tick, counters: { ...counters } })),
          version: RELATIONSHIP_VERSION, memoryVersion: MEMORY_CONTRACT_VERSION,
          configHash, personaHashes,
      ...
      this.relationshipSamples.delete(room.id);
      // The relation source is not the personality source. Keep personality
      // available while leaving all unverified memory and treatment hashes absent.
      try { await samplePersonas(); } catch { personas = {}; }
      return {personas};
    }
    ...
    // Whether this snapshot may be appended is the central rule's question, not
    // this caller's: the flush refuses an entry whose room object a settled
    // restore replaced, and it waits out a restore still in flight, so a
    // snapshot derived from a replaced room cannot reach the log that replaced
    // it. Restating the identity check here would be the second place the rule
    // lives, which is what the central check exists to remove.
    this.#queueAudit(room, () => event, () => true);
    return { derived, appraisals, personal, personas, configHash };
  }

  /**
   * Append the automatic `clear` that opens one episode under the
   * `reset_per_episode` arm.
   *
   * It is an ordinary intervention event, not a private reset: it carries the
   * counters as they stood before it, and its `mechanism` is
   * `arm:reset_per_episode` with `appliedBy: "arm"`, so a reader can tell at a
   * glance that the room's own arm mechanism applied it rather than a human. It
   * also states the `episodeId` (the root message) it opened, which is what lets
   * a later turn of the same episode — the retry of a failed delivery — see that
   * its reset is already in the log and not append a second one.
   * The append is awaited rather than queued behind the next save because the
   * derivation that follows must be able to read it: it describes no room state
   * beyond the tick that is already durable, so ordering it before the save is
   * not the R20 hazard that queued appends exist for. A failed append returns
   * `null` and the episode simply runs with the arm's reset missing, which is
   * the same degradation every other unreadable observation takes; the failed
   * append is counted in the log's health either way.
   */
  async #appendEpisodeReset(room, events, asOfTick, episodeId) {
    const before = deriveRelationships({ events, roomId: room.id, asOfTick });
    const countersBefore = {};
    for (const pair of before.pairs) {
      // One row per target: the counters are the target's own record, so the
      // self-row is the complete "before" under every observer.
      if (pair.observer !== pair.target) continue;
      countersBefore[pair.target] = { ...pair.counters };
    }
    return await this.journal.appendChecked(room, {
      type: RELATIONSHIP_INTERVENTION_EVENT_TYPE,
      actor: { kind: "system", id: "system" },
      payload: { action: "clear", observerId: null, targetId: null, counters: null, countersBefore,
        appliedBy: "arm", mechanism: ARM_RESET_MECHANISM, note: null, episodeId,
        memoryScope: "all", memoryVersion: MEMORY_CONTRACT_VERSION },
      provenance: { originClass: "system", sessionKind: "interactive" }
    });
  }
  ...
  async prepareMember(roomId,sessionId) {
    ...
    const validate=()=>{
      if(this.closed||room.deletedAt||!member||this.#groupInactive(room)||room.epoch!==epoch
        ||this.state.rooms.find(item=>item.id===roomId)!==room||!room.members.includes(member)) {
        throw new Error("准备期间对话或成员已变化；未发送任务。");
      }
    };
    validate();
    if((!member.nativeSetup||member.nativeSetup.state==="ready")&&member.nativePermissionSync?.state!=="pending")return copy(member);
    // A shared native Session can have distinct pending work in each room.
    // Coalesce only work that actually completed this member's preparation.
    for(let existing=this.sessionPreparations.get(sessionId);existing;existing=this.sessionPreparations.get(sessionId)){
    ...
    const promise=(async()=>{
      if(member.nativePermissionSync?.state==="pending"){
        ...
        const validatePermission=()=>{validate();if(room.policy.defaultActionMode!==mode)throw new Error("原生权限待同步設定已過期；請在權限面板重新確認");};
        validatePermission();
      ...
      await provisionConversationSession(this.ctx,{...member,sessionTitle:`${room.name} · ${member.alias}`.slice(0,120)},validate);
      validate();
      if(room.policy.defaultActionMode!=="inherit_dsh") {
        const mode=room.policy.defaultActionMode;
        const validatePermission=()=>{validate();if(room.policy.defaultActionMode!==mode)throw new Error("对话权限已变化，未应用权限。");};
    ...
    try{await promise;validate();return copy(member);}finally{if(this.sessionPreparations.get(sessionId)===promise)this.sessionPreparations.delete(sessionId);}
  }
  ...
  async selectMemberModel(roomId,sessionId,selection) {
    await this.ready;
    const room=this.#room(roomId),member=room.members.find(item=>item.sessionId===sessionId);
    const epoch=room.epoch,agentId=member?.agentId;
    const validate=()=>{this.#assertIdleForConfiguration(room);if(this.closed||room.deletedAt||this.#groupInactive(room)
      ||this.state.rooms.find(item=>item.id===roomId)!==room||room.epoch!==epoch||!room.members.includes(member)||member.agentId!==agentId)throw new Error("对话或成员已变化，请重新选择。");
    ...
    validate();
    if(this.modelSelections.has(sessionId))throw Object.assign(new Error("此原生會話的模型正在修改；請完成後重新核對。"),{status:409});
    this.modelSelections.add(sessionId);
    try {
      await this.prepareMember(roomId,sessionId);validate();
      ...
      return {selected};
    } finally {this.modelSelections.delete(sessionId);}
  }
  ...
  /** Read committed state and its settled events without mutable-state leakage. */
  async readRoomMemory(roomId) {
    await this.ready;
    this.#room(roomId);
    return this.journal.readRoomMemory(roomId);
  }

  /** Snapshot one room together with its immutable log and the run's config hash. */
  async snapshotRun(roomId, configHash) {
    const { room, events } = await this.readRoomMemory(roomId);
    return exportRunSnapshot({ room, version: STATE_VERSION, events, configHash });
  }

  /** Restore the room and its log under the journal's per-room ordering. */
  async restoreFromSnapshot(snapshot, { confirm } = {}) {
    await this.ready;
    if (this.closed) throw new Error("group-chat service is closed; snapshot restore was not started");
    ...
    return this.journal.runRestore(() => this.journal.runRoom(checked.snapshot.room.id, () => this.#restoreLocked(checked)));
  }

  async #restoreLocked(checked) {
    if (this.closed) throw new Error("group-chat service is closed; queued snapshot restore was not started");
    ...
    await this.#backupState(`${this.path}.pre-restore.bak`).catch((error) => {
      if (error?.code !== "ENOENT") throw error;
    });
    if (this.closed) throw new Error("group-chat service is closed; snapshot restore was not installed");
    if (this.state.rooms[index] !== previous) throw new Error("room changed during snapshot backup; restore was not installed");
    const restored = this.#normalizeRoom(checked.snapshot.room, []);
    // Validate inferred legacy bindings before interrupting the current room.
    const candidate = copy(this.state);
    candidate.rooms[index] = copy(restored);
    migrateAgentDirectory(candidate);
    for (const room of candidate.rooms) assertDistinctRoomIdentities(room);
    previous.epoch += 1;
    ...
    migrateAgentDirectory(this.state);
    await this.journal.restore(restored, checked.snapshot.events, this.state, {
      rollback: () => { this.state.rooms[index] = previous; },
      beforeReplace: () => { this.relationshipSamples.delete(restored.id); }
    });
    return { roomId: restored.id, eventsWritten: checked.snapshot.events.length };
  }

  async #backupState(destination) {
    const bytes=await readFile(this.path);
    return this.capacity.run({peakBytes:bytes.length + 8192}, async () => {
  ...
  async saveRoomExport(roomId, format = "json") {
    ...
    if (this.closed) throw new Error("group-chat service is closed");
    return this.capacity.run({peakBytes:Buffer.byteLength(exported.content)+8192}, async () => {
    if (this.closed) throw new Error("group-chat service is closed");
    ...
    try {
      ...
      created = true;
      try { await file.writeFile(exported.content, "utf8"); await file.sync(); }
  ...
  #memoryMember(room, sessionId, requireTurn = false) {
    this.#recallRoom(sessionId, room.id);
    const member = room.members.find((item) => item.sessionId === sessionId);
  ...
  async roomMemory(roomId, sessionId) {
    ...
    const result = copy({ roomId: room.id, groupId:room.groupId, conversationTitle:room.name, ...(room.origin?{origin:room.origin}:{}), profile: room.profile, history: sessionId === undefined ? room.profileHistory : room.profileHistory.slice(-3),
    ...
    if (sessionId !== undefined) {
      const items = [...result.ledger.map(item => ({ id: item.id, kind: "ledger", text: JSON.stringify(item) })),
        ...result.proposals.map(item => ({ id: item.id, kind: "charter-proposal", text: JSON.stringify(item) })),
        { id: `profile:${result.profile.revision}`, kind: "charter", text: JSON.stringify(result.profile) }]
        .map(item => ({ ...item, sourceChars: item.text.length, truncated: item.text.length > 2000, text: item.text.slice(0, 2000) }));
      const observed = await this.#recordObservation(room, sessionId, { origin: "chat_memory",
        messageIds: result.messages.map(item => item.id), evidenceIds: items.map(item => item.id), items });
      result.memoryReceipt = { status: observed ? "recorded" : "unavailable", observationId: observed?.id ?? null };
    }
    return result;
  }
  ...
  async setRoomPolicy(roomId, { defaultActionMode, expectedRevision, confirmRisk, gate }) {
    ...
    // Naming an execution mode takes an explicit risk confirmation, and it did
    // so before this call could carry anything but a mode. That is unconditional
    // and it stayed unconditional: a call that merely re-sends the mode already
    // in force names an execution mode too, and it refused before this call
    // could move the gate. The one exception is a call that moves the gate and
    // nothing else inside a mode the user has already confirmed: the gate is not
    // a permission, and that call names no execution change at all.
    const gateOnlyChange = gateChanged && !modeChanged && !presetMode;
    if (EXECUTION_MODES.has(mode) && confirmRisk !== true && !gateOnlyChange) {
    ...
    room.roomSeq += 1;
    // The notice states what this call actually changed. A call may move the
    // mode, the gate, both, or — when it re-applies a native preset to a room
    // already in that mode — neither, and a record that named a change that did
    // not happen would be a false fact in the room's own timeline.
    const changes = [];
    if (modeChanged) changes.push(`用户将房间权限切换为 ${mode}。${mode === "full_access" ? "现有成员的原生 DSH 会话已设为 danger-full-access / never；允许命令与文件修改，不再额外审批。这些会话的原生设置也会保留在群聊之外。" : mode === "workspace_write" ? "现有成员原生 DSH 权限已设为 workspace-write / ask。" : mode === "inherit_dsh" ? "不改会话权限，由 DSH 原生沙箱与审批控制。" : "群聊恢复只读限制；不会静默重写原生会话权限。"} 不自动发送任务或宣称阻断已解除。`);
    if (gateChanged) changes.push(`用户${nextGate ? "开启" : "关闭"}了本房间的行动治理门。开启时，执行类回合里的高影响动作要先对照本房间的事件记录：记在该成员名下的投递失败或未闭环分歧会直接拒绝执行并说明原因，不会弹出审批。关闭时不改变任何工具的执行结果。`);
    if (changes.length === 0) changes.push(`用户重新应用了房间权限设置（${mode}）。本房间的策略与执行权限没有变化。`);
  ...
  async addMember(roomId, member, { expectedRevision } = {}) {
    ...
    normalized.alias = this.#uniqueAlias(room, normalized.alias, normalized.sessionId);
    assertDistinctRoomIdentities({ ...room, members: [...room.members, normalized] });
    room.members.push(normalized);
  ...
  async listParticipants(roomId) {
    ...
    return await Promise.all(room.members.map(async (member) => {
      if(member.nativeSetup?.state==="pending")return copy({...member,runtime:{state:"unprepared"},nativeContext:this.nativeContextStatus(member.sessionId)});
      ...
      return copy({ ...member, runtime, nativeContext:this.nativeContextStatus(member.sessionId), sharedWith,sharedNative:member.ownership!=="provisioned"||sharedWith.length>0,...(nativePermission ? { nativePermission } : {}) });
    }));
  }

  /** Historical transport bindings never silently identify a successor as the owner. */
  #historicalParticipantIdentity(room, sessionId) {
    const identities = new Set(this.state.workspace.participations
      .filter(item => item.roomId === room.id && item.sessionId === sessionId && item.agentId)
      .map(item => item.agentId));
    const current = room.members.find(member => member.sessionId === sessionId)?.agentId;
    if (current) identities.add(current);
    if (identities.size !== 1) throw new Error("independent review requires an unambiguous historical Agent identity; reassign the task explicitly");
    return [...identities][0];
  }

  #assertIndependentReviewer(room, entry, reviewerSessionId) {
    const reviewer = room.members.find(member => member.sessionId === reviewerSessionId)?.agentId
      ?? this.#historicalParticipantIdentity(room, reviewerSessionId);
    const submitter = entry.submission?.agentId
      ?? this.#historicalParticipantIdentity(room, entry.ownerSessionId);
    if (reviewer === submitter) throw new Error("task owner cannot review their own submission through another Session; an independent Agent is required");
  }
  ...
  #validateLedgerReferences(room, entry, previous) {
    ...
    if (entry.kind === "task" && entry.ownerSessionId && entry.reviewerSessionId
      && (!previous || entry.ownerSessionId !== previous.ownerSessionId || entry.reviewerSessionId !== previous.reviewerSessionId)) {
      this.#assertIndependentReviewer(room, entry, entry.reviewerSessionId);
    }
    ...
    switch (input.action) {
      ...
      case "submit":
        ...
        if (input.action === "progress") {
          ...
          patch = { status: "in_review", submission: { summary, deliverable, actor, agentId: member.agentId, at: Date.now(), sources, revision: current.revision + 1 }, blocker:null, review: null };
        }
        break;
      case "review":
        ...
        this.#assertIndependentReviewer(room, current, sessionId);
  ...
  async readDocument(roomId, sessionId, input) {
    ...
    const response = { artifact: result.artifact, contentHash, access: result.access, extraction: result.extraction,
    ...
    const observed = await this.#recordObservation(room, sessionId, { origin: "chat_read_document", items: [{
      id: `document:${contentHash}:${startLine}-${next - 1}`, kind: "document-excerpt", text: response.content.slice(0, 20_000),
      contentHash, startLine, endLine: next - 1, sourceChars: response.content.length,
      storedChars: Math.min(response.content.length, 20_000), truncated: response.content.length > 20_000 }] });
    response.memoryReceipt = { status: observed ? "recorded" : "unavailable", observationId: observed?.id ?? null };
    return response;
  }
  ...
  guardToolExecution(execution) {
    ...
    if (isReadOnlyTool(name)) {
      const denial = this.#restrictedReadDenial(name, execution.arguments, execution.agent?.session?.header?.cwd);
  ...
  #actionGate(lock, sessionId, name, args) {
    ...
    const judgement = evaluateGate({ pair, action: classification.action,
      riskClass: classification.riskClass, unclassified: classification.unclassified === true,
      policy: room.policy });
   ...
   */
  #restrictedReadDenial(name, args, cwd) {
    if (referencesPrivateState(args, { statePath: this.path, cwd, urlArguments: EGRESS_TOOL_NAMES.has(name) })) {
  ...
  async send({ roomId, author, authorAlias, text, mentions, authorKind, clientOperationId, correctsMessageId, automaticDelivery = true, resolveTextMentions = true, handoffReference, fileShareReference, retrySource }) {
    ...
    // `handoffEntry` is absent whenever the reference names no entry of this
    // room, and `handoffReference.id` is absent on a hand-built reference: with
    // both undefined the first comparison is vacuously false and
    // `handoffEntry.handoff` dereferenced nothing. Optional chaining keeps the
    // intended refusal (`... !== "sending"` is true for a missing entry) instead
    // of throwing a TypeError from inside the guard.
    ...
    const message = {
      ...
      ...(normalizedAuthorKind === "session" && member?.agentId ? { authorAgentId: member.agentId } : {}),
  ...
  async #commitSend(messageId) {
    ...
    receipt.committing=(async()=>{
      await this.#save();
      if (this.state.rooms.find(item => item.id === room.id) !== room || !room.messages.includes(message)) {
        throw new Error("message was superseded by a restored snapshot");
      }
    ...
    try { await receipt.committing; } finally {
      receipt.committing=null;
      // A restore can finish while this send awaits an unrelated room's audit.
      // Such a send is no longer retryable; retaining its old room would allow
      // a later request to resume work belonging to the discarded history.
      if (this.state.rooms.find(item => item.id === room.id) !== room) {
        delete message.savePending;
        this.pendingSends.delete(message.id);
      }
    }
  ...
  /**
   * The bounded relationship digest this member's prompt carries.
   *
   * Rendered from the turn's own snapshot derivation and its appraisals, never
   * from a fresh read or a per-member `relationships()` call: all members of a
   * turn share one sample, so what a member reads cannot contradict the snapshot
   * recorded for that turn. `null` (no readable state, or nothing recorded yet,
   * or nothing that fits) injects nothing at all rather than an empty heading.
   *
   * The judgement half is dropped when this process has it switched off; the
   * counters always travel. The switch is a term of the run manifest's config
   * hash, so a run with it off is not compared against one with it on.
   *
   * Handed out separately from `#participantPrompt` because the caller records
   * its exact length and hash as this turn's injection cost: the value recorded
   * is the value injected, not a re-rendering of it.
   */
  #turnDigest(room, member, turnSample) {
    const aliases = new Map(room.members.map((item) => [item.sessionId, item.alias]));
    return renderRelationshipDigest({ derived: turnSample?.derived, observer: member.sessionId,
      appraisals: this.appraisalDigest ? turnSample?.appraisals : undefined,
      labelOf: (sessionId) => aliases.get(sessionId) });
  }

  #participantPrompt(room, member, task, digest, personalDigest, persona) {
    ...
    return [
      ...
      ...(persona?.configured ? [`你的持續身分是 ${member.agentId}。以下人格描述你的性格與習慣；不覆蓋使用者當前要求、房間權限或事實證據。\n<agent_persona>\n${persona.markdown}\n</agent_persona>`] : []),
      ...roomContext,
      ...
      `可对话的其他参与者：${peers}。`,
      "本房間同事與台帳請用 chat_memory、chat_work 和精確 @別名。DSH 的 list_agents、send_message、team_task_*、wait_agent 屬於目前原生會話自己的 Agent Teams 子團隊，不代表本房間，也不會同步房間台帳；不要用它們尋找或催促群聊同事。只有使用者明確要求另外委派子團隊時才使用該協作流程。",
      ...(digest ? [digest] : []),
      ...(personalDigest ? [personalDigest] : []),
      "你可用 chat_identity 查看本人持續身分與人格，用 chat_recall 查詢自己實際看過的跨工作經歷、目前信念及本人評價；用 chat_memory_update 明確釘選、抑制或修訂有原始證據的本人信念。記憶是背景，不能把旧任務當成目前指令；別人的私有評價不屬於你的記憶。",
  ...
  async #runTurn(roomId, rootMessageId, epoch, recipientIds) {
    ...
    this.#queueAudit(room, () => ({ type: "turn.scheduled",
      actor: { kind: "system", id: "system" },
      ...
      provenance: { originClass: "system", sessionKind: "interactive" } }), () => room.messages.includes(rootMessage));
    await this.#save();
    ...
    // snapshot cannot feed back into who runs. The counters and the appraisals
    // are kept together and handed to every delivery below, so the whole turn
    // injects one sample.
    const turnSample = await this.#auditTurnSnapshot(room, rootMessageId);
    ...
    while (queue.length > 0 && visibleReplies < this.maxReplies) {
      ...
      const result = await this.#deliverOne(room, rootMessage, member, epoch, task, turnSample);
  ...
  #newCapture(room, rootMessage, member, epoch, task, delivery) {
    ...
    // Registration permits cancellation during preparation. The recipient's
    // reply clock starts only when its completed prompt is dispatched.
    return capture;
  }

  #startReplyTimer(room, capture) {
  ...
  async #deliverOne(room, rootMessage, member, epoch, task, turnSample) {
    const bridge = this.#bridge();
    const delivery = {
      id: crypto.randomUUID(),
    ...
    const assertCurrent = () => {
      if (this.closed || room.deletedAt || this.#groupInactive(room) || room.epoch !== epoch
        || capture.settled || this.state.rooms.find(item => item.id === room.id) !== room) {
        throw new Error("对话已停止或切换了执行回合；未唤醒成员。");
      }
    };
    // Native services and bridge transports can outlive their caller. Their
    // promises remain observed, but a completed capture releases this run;
    // late external results cannot continue toward another dispatch.
    const external = operation => Promise.race([
      Promise.resolve().then(() => { assertCurrent(); return operation(); }).then(value => ({ value })),
      capture.promise.then(result => ({ completed: true, result }))
    ]);
    try {
      if (!bridge) throw new Error("dsh-bridge is not installed; local delivery unavailable");
      const prepared = await external(() => this.prepareMember(room.id, member.sessionId));
      if (prepared.completed) return prepared.result;
      if (!EXECUTION_MODES.has(capture.actionMode)) this.hostCompatibility.assertDeliverySupported(this.#agents()?.get?.(member.sessionId));
      const models = await external(() => this.#runModels({ members: [member] }));
      if (models.completed) return models.result;
      const modelAtDelivery = models.value[member.sessionId];
      assertCurrent();
      const rendered = this.#turnDigest(room, member, turnSample);
      const digest = rendered.text;
      const personal = renderPersonalMemory(turnSample?.personal?.[member.sessionId], { includeJudgements: this.appraisalDigest });
      const persona = turnSample?.personas?.[member.sessionId];
      const promptText = this.#participantPrompt(room, member, task, digest, personal.text, persona);
      capture.observedMessageIds = room.messages.slice(-24).map(message => message.id);
      capture.observerAgentId = member.agentId;
      ...
      await this.#record(room, { type: "turn.prompt",
        ...
        provenance: { originClass: "system", sessionKind: "interactive" } });
      assertCurrent();
      // What the relationship injection cost, recorded from the value that was
      // just injected rather than from a ceiling: `digestChars` is the digest
      // string's own length and `digestHash` identifies it, so a reader can
      // re-derive the turn's digest and check both. The token figure is an
      // estimate and says so, with the code-point counts it was taken from, so
      // it cannot be mistaken for a measurement. Recording this changes neither
      // the injected text nor the cap.
      const estimate = estimateTokens(digest ?? "");
      await this.#record(room, { type: INJECTION_COST_EVENT_TYPE,
        actor: { kind: "session", id: member.sessionId },
        payload: { deliveryId: capture.id, memberSessionId: member.sessionId,
          digestChars: digest === null ? 0 : digest.length,
          digestHash: digest === null ? null : createHash("sha256").update(digest).digest("hex"),
          digestMetadata: rendered.metadata, personalMemoryMetadata: personal.metadata,
          personalMemoryChars: personal.text?.length ?? 0,
          personalMemoryStatus: turnSample?.personal?.[member.sessionId]?.status ?? "unavailable",
          personaHash: persona?.hash ?? null, personaChars: persona?.configured ? persona.markdown.length : 0,
          configHash: turnSample?.configHash ?? null,
          memorySampleStatus: turnSample ? "available" : "unavailable",
          modelAtDelivery, modelObservation: "before-delivery",
          promptChars: promptText.length,
          personalEstimatedTokens: estimateTokens(personal.text ?? "").tokens,
          personaEstimatedTokens: estimateTokens(persona?.configured ? persona.markdown : "").tokens,
          estimatedTokens: estimate.tokens, tokenEstimateMethod: estimate.method, tokenEstimateExact: false,
          tokenEstimateCjkChars: estimate.cjkChars, tokenEstimateOtherChars: estimate.otherChars },
        provenance: { originClass: "system", sessionKind: "interactive" } });
      assertCurrent();
      this.#startReplyTimer(room, capture);
      const dispatch = await external(() => bridge.deliverExternal(
        `room:${room.id}`,
        member.sessionId,
        promptText,
        { id: capture.id, transport: ROOM_TRANSPORT }
      ));
      if (dispatch.completed) return dispatch.result;
      if (!capture.settled) {
      ...
      try { await this.#save(); }
      finally { this.#finishCapture(capture, { status: "failed", error: message }); }
    }
  ...
  /**
   * Whether a queued delivery transition is still owed to the log once its save
   * has landed: the delivery must still hang off one of the room's messages, so
   * a message dropped before the write is not recorded.
   *
   * The room-identity question is deliberately *not* asked here. It used to be
   * (`this.state.rooms.find(...) !== room`), and the restore path was exactly
   * what it was for; the flush now asks it once for every queued entry, before
   * any `stillValid` runs, so restating it here would be a second place a future
   * event type had to remember and a rule this predicate could not enforce for
   * anyone else's entries. A flush reaching this predicate has already settled
   * that `room` is the live object, and it settles it synchronously with the
   * append that follows, so nothing can detach it in between.
   *
   * The status this event names is deliberately *not* part of this check either.
   * It was settled when the save claimed the transition, against the snapshot it
   * was about to write, because the delivery object is mutated in place: by flush
   * time a delivery the save did persist as `sent` may already read `delivered`,
   * and re-reading it here would drop a transition that really is on disk — an
   * irrecoverable hole in the record `deliveriesOffered` and every relationship
   * snapshot are counted from.
   */
  #deliveryPersisted(room, delivery) {
   ...
   * state no longer holds is dropped rather than appended. "Holds" is decided at
   * claim time — the instant the snapshot that would carry the status is
   * serialised — because the delivery is mutated in place and its later statuses
   * must not retract an earlier transition the save did write.
   */
  #setDelivery(room, capture, status, extra = {}) {
    ...
    this.#queueAudit(room, () => this.#deliveryEvent(messageId, delivery, status, reported),
      () => this.#deliveryPersisted(room, delivery),
  ...
  #finishCapture(capture, result) {
    if (capture.settled) return;
    this.hostCompatibility.finishCapture(capture);
  ...
  #captureFromMarker(sessionId, marker) {
    const capture = this.pending.get(marker.deliveryId);
    const room = this.state.rooms.find(item => item.id === marker.roomId);
    if (!capture || capture.settled || capture.sessionId !== sessionId || capture.roomId !== marker.roomId
      || !room || room.deletedAt || this.#groupInactive(room) || room.epoch !== capture.epoch
      || !room.members.some(member => member.sessionId === sessionId
        && (!capture.observerAgentId || member.agentId === capture.observerAgentId))
      || !room.messages.some(message => message.id === capture.rootMessageId && message.deliveries.includes(capture.delivery))) return undefined;
    return capture;
  }

  #rejectDeliveryMarker(sessionId, marker) {
    // Restore can legitimately erase the delivery record. A reserved bridge
    // marker without a current matching capture is not a native permission
    // grant, even after remount when turn/start was never observed. This lock
    // is released by turn/end or a later turn/start, not retained as a tombstone.
    this.policyLocks.set(sessionId, { captureId: marker.deliveryId, roomId: marker.roomId,
      turn: this.turnBySession.get(sessionId), active: true, stale: true, ambiguous: true, actionMode: "discuss_only" });
    try { this.#agents()?.get?.(sessionId)?.cancel?.(
      { kind: "hook", reason: "unverifiable or expired group-chat inbox delivery" }, { keepInbox: true }); }
    catch { /* the installed guard still prevents execution */ }
  }
  ...
  async observeSessionEvent(sessionId, event) {
    ...
    if (this.closed) return;
    const id = String(sessionId);
    this.observationCoverage.event(event);
    if (event?.type === "tool/call") {
      this.observedToolCalls.set(JSON.stringify([id, event.data?.callId]), { sessionId: id, name: event.data?.name });
      return;
    }
    if (event?.type === "tool/result") {
      const callId = event.data?.message?.source?.callId ?? event.data?.message?.toolCallId
        ?? event.data?.message?.content?.[0]?.toolCallId;
      const key = JSON.stringify([id, callId]);
      const tool = this.observedToolCalls.get(key)?.name;
      this.observedToolCalls.delete(key);
      // Reading a memory is not an independent experience supporting itself.
      // These tools already issue structured receipts where appropriate.
      if (["chat_identity", "chat_recall", "chat_memory_update", "chat_memory", "chat_relationships", "chat_appraise", "chat_read_document"].includes(tool)) {
        this.observationCoverage.count("excludedRecallEvents"); return;
      }
    }
    if(["permission/preset","sandbox/mode","approval/policy"].includes(event?.type))this.permissionReads.delete(id);
    if (event?.type === "turn/start") {
      this.nativeContextSamples.delete(id);
    ...
    if (event?.type === "user/message") {
      const markers = [...eventText(event).matchAll(/\[dsh-bridge\s+dsh-chat-local-room\s+message\s+([^\s\]]+)\s+from\s+room:([^\s\]]+)\]/gu)]
        .map(match => ({ deliveryId: match[1], roomId: match[2] }));
      if (!markers.length) { await this.#observeNative(id, event); return; }
      const identities = new Set(markers.map(marker => JSON.stringify([marker.deliveryId, marker.roomId])));
      const capture = identities.size === 1 ? this.#captureFromMarker(id, markers[0]) : undefined;
      if (!capture) {
        this.#rejectDeliveryMarker(id, markers[0]);
        return;
      }
      ...
      //
      // `mixed` claims a lock already tracks a *different* capture in the same
      // turn, so it needs a lock to compare against. An absent `previous` (a
      // remount that never saw the running turn's `turn/start`) and an untracked
      // `capture.turn` are both `undefined`, so `previous?.turn===capture.turn`
      // was vacuously true and the next term dereferenced nothing: the marker
      // observation threw before it wrote the `delivered` settle or the lock that
      // arms the room's tool guard. With no previous lock there is nothing to be
      // mixed with, and `false` is what the three terms below already mean for a
      // missing `previous`: `ambiguous` falls back to `previous?.ambiguous` (which
      // still propagates an ambiguity an earlier lock established), `stale` to
      // false, and `actionMode` to the capture's own.
      const mixed=previous!==undefined&&previous.turn===capture.turn&&previous.captureId!==capture.id;
      this.policyLocks.set(id,{captureId:capture.id,roomId:capture.roomId,turn:capture.turn,active:true,
        ...
        stale:previous?.turn===capture.turn&&previous?.stale,
      ...
      // Receipt is an external observation, already true before state persistence.
      // Register it before yielding so the first model tool joins this log write.
      const observing = this.#recordObservation(this.#room(capture.roomId), id, { origin: "delivery", deliveryId: capture.id,
        observerAgentId: capture.observerAgentId, messageIds: capture.observedMessageIds ?? [] });
      observing.catch(() => {});
      await this.#save();
      await observing;
      return;
    ...
    if (event?.type === "turn/end") {
      this.nativeContextSamples.delete(id);
      if (this.turnBySession.get(id) === turn) this.turnBySession.delete(id);
      for (const [key, value] of this.observedToolCalls) if (value.sessionId === id) this.observedToolCalls.delete(key);
    }
    ...
    // A turn this service never observed is not a turn it can attribute an event
    // to. `#newCapture` leaves `capture.turn` undefined until the marker is
    // observed, so an event that names no turn compared equal to an untracked
    // capture's own `undefined` and settled it: one `assistant/message` (or
    // `turn/end`) without a `turn` was enough to write a reached status for a
    // delivery the member's session never reported receiving. A valid DSH stream
    // always names the turn; refusing the match is what keeps every reached
    // status downstream of the marker observation that proves reach.
    const capture = turn === undefined ? undefined
      : [...this.pending.values()].find((item) => item.sessionId === id && item.turn === turn);
    if (!capture) {
      await this.#observeNative(id, event);
      if (event?.type === "turn/end") {
        const lock = this.policyLocks.get(id);
    ...
    if (event.type === "tool/result") {
      const lock = this.policyLocks.get(id);
      const room = this.state.rooms.find(item => item.id === capture.roomId);
      if (room && room.epoch === capture.epoch && !capture.settled && !lock?.ambiguous && !lock?.stale) {
        const items = this.#sessionObservationItems(id, event);
        if (items.length) await this.#recordObservation(room, id, { origin: "tool-result",
          observerAgentId: capture.observerAgentId, items });
      }
      return;
    }
    ...
    if (event.type === "assistant/message") {
      ...
      // `working` means the member started working on the delivered prompt, so it
      // may only be entered from the settle the member's own session produced.
      // `sent` says the transport accepted the prompt, not that anyone saw it;
      // admitting it let a single event move a delivery straight from `sent` to a
      // reached status. A matched capture carries a turn, which only the marker
      // observation sets, so `delivered` is the state this path already runs in.
      if (capture.delivery.status === "delivered") {
    ...
    if (stale) {
      ...
      const message = {
        ...
        authorKind: "session",
        authorAgentId: capture.observerAgentId,
