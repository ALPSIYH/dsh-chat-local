...
import {
  // Every term below is also a term of `injectionConfigFor`'s hash: this module
  // reads them from one place rather than keeping a second copy that could
  // change the injected text without changing the config hash.
  RELATIONSHIP_DIGEST_MAX_CHARS, RELATIONSHIP_DIGEST_CLAIM_MAX_CHARS, RELATIONSHIP_DIGEST_HEADING,
  RELATIONSHIP_DIGEST_APPRAISAL_HEADING, RELATIONSHIP_DIGEST_COUNTERS, RELATIONSHIP_DIGEST_STANCES,
  RELATIONSHIP_DIGEST_COUNTER_TEMPLATE, RELATIONSHIP_DIGEST_APPRAISAL_TEMPLATE,
  RELATIONSHIP_DIGEST_ROLE_TEMPLATE, RELATIONSHIP_DIGEST_FIELD_TEMPLATE, RELATIONSHIP_DIGEST_COUNTER_JOIN,
  fillTemplate, injectionConfigFor, configHashOf, runArm, RUN_MANIFEST_EVENT_TYPE, RUN_ARMS,
...
/**
 * The digest's terms and its cap live in `lib/experiment.js`, which is also
 * where `injectionConfigFor` hashes them: the injected text and the hash that
 * decides whether two runs are comparable are built from one set of constants,
 * so a term cannot change the text without changing the hash. Re-exported here
 * because this module is where the digest is rendered and where callers have
 * always read the cap from.
 */
export { RELATIONSHIP_DIGEST_MAX_CHARS };
...
export class DshChatLocalService {
   ...
   * - **What the writer stamps.** `validFrom` is the room's current tick, so the
   *   statement is in force from the tick it was made on, and the record is
   *   written through the audit queue like every other event.
   */
