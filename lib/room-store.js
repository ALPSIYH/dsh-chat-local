...
export class DshChatLocalService {
  constructor(ctx, config = {}) {
    ...
    // log's own tail, keyed by the exact room object the replace belongs to and
    // carrying that object so a lookup by room id is possible too. A flush waits
    // on it before asking anything about an entry: an append the replace would
    // erase is then ordered after it, and an entry for the object the replace
    // detached is refused only once the restore has settled. The save that owns
    // the gate skips it, which is what keeps the wait acyclic — the restore's own
    // save is waiting for that flush to end, and no entry belonging to the object
    // it installs can have been claimed by it.
   ...
   *
   * `replacing` is the log-replace gate this save owns, set only by
   * `restoreFromSnapshot`: the flush must not wait on the replace its own save
   * has to release, because that restore is waiting for this flush to end.
   */
  async #save(replacing = null) {
    ...
    const flush = (async () => {
      for (const { entry, inSnapshot } of audited) {
        ...
        // Every log replace still in flight for this entry's room is waited out
        // before anything is asked about the entry, because both directions of
        // the central rule below need that replace to have settled first.
        //
        ...
        // repeated until nothing is in flight: a later restore of the same room
        // publishes its own replace, and either of them may be the one that
        // detaches this object or brings it back.
        //
        // The replace this save itself owns is skipped. A flush runs only once
        // its own state write has landed, so the save that owns a replace can
        // no longer be rolled back; and the restore owning that replace is
        // waiting for this very flush to end, so waiting on it would be waiting
        // on itself. An entry belonging to the object that restore installed
        // cannot be in this save's claim: the object is created, installed and
        // the audit queue claimed in one synchronous step, so nothing else can
        // have queued against it by then.
        let replace = this.#pendingLogReplaceFor(entry.room, replacing);
        while (replace) {
          await replace.promise;
          replace = this.#pendingLogReplaceFor(entry.room, replacing);
        }
  ...
  /**
   * The log replace still in flight for one entry's room, if any, other than the
   * one this save owns.
   *
   * A replace is keyed by the room object it installs, but the question here is
   * about the room id: the object an entry belongs to may be the one a restore
   * detached, and only a replace of that same id can bring it back. Matching on
   * the id and skipping the save's own gate keeps a flush from waiting on a
   * replace it must itself release, which is what keeps the wait acyclic.
   */
  #pendingLogReplaceFor(room, owned) {
    for (const gate of this.pendingLogReplace.values()) {
      if (gate !== owned && gate.room.id === room?.id) return gate;
    }
    return null;
  }
  ...
  async restoreFromSnapshot(snapshot, { confirm } = {}) {
    ...
    // the replace rather than be erased by it. A flush also waits on it before
    // refusing an entry for the object this restore detached, so the refusal is
    // decided against a settled restore rather than a swap that may still be
    // rolled back below. See the flush in `#save`.
    const logReplace = { room: restored, promise: null, release: null };
    logReplace.promise = new Promise((resolve) => { logReplace.release = resolve; });
    this.pendingLogReplace.set(restored, logReplace);
    try {
      await this.#save(logReplace);
    } catch (error) {
    ...
    // which is a legitimate append that must survive: this release is what puts
    // it after the replace instead of inside what the replace erases.
    try {
