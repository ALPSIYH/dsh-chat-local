...
export class DshChatLocalService {
  constructor(ctx, config = {}) {
    ...
    // log's own tail, keyed by the exact room object the replace belongs to. A
    // flush whose entry is about that object waits on it, so an append the
    // replace would erase is instead ordered after it. Keying on the object and
    // not the room id is what keeps the wait acyclic: only an entry belonging to
    // the restored object can wait, and the restore's own save — the only save
    // running while the replace is pending — cannot have claimed one.
    this.pendingLogReplace = new Map();
  ...
  async #save() {
    ...
    // audited by a save whose snapshot does not yet contain it. The claim also
    // settles, synchronously and against the state just serialised, whether this
    // snapshot still holds the fact each event names (`inSnapshot`). That
    // question cannot be deferred to the flush: the objects the events describe
    // are mutated in place, so by the time the write lands a delivery may have
    // moved on, and a transition this save really did write would be dropped
    // from the log forever. `stillValid` stays at flush time for the one fact
    // only the written state can settle — that the room this save wrote is still
    // the live room — and the flush asks it for every entry itself, together with
    // whether that room's log is mid-replace, so no `stillValid` has to restate
    // either question.
    ...
    // cannot reject: the state is durable by now, and a later `settledAudit` is a
    // read that must report the log rather than inherit a failure from a save it
    // never made. This save still sees the failure below.
    const flush = (async () => {
      for (const { entry, inSnapshot } of audited) {
        ...
        // The one central check, asked here for every queued entry so that no
        // future event type has to restate it in its own `stillValid`. It has
        // two halves, in this order.
        //
        // First the identity of the room object the fact belongs to: it is the
        // only discriminator there is, because a restore keeps the room id and
        // changes the object. An entry queued against the object a restore
        // discarded is refused here whether the flush is asked before that
        // object is detached or long after.
        if (!this.#roomStillLive(entry.room)) continue;
        // Then the log that object was created against. A restore replaces a
        // room's whole log, so an append registered before the replace is erased
        // by it — and identity alone cannot order the two: a save claimed after
        // the swap passes the check above, and the restore joins that save's
        // write but not its flush, so the append can still land first. The
        // replace is published synchronously at the swap, before anything is
        // written, and keyed by the object it belongs to, so waiting here is
        // waiting for that object's replace to be registered and no other
        // entry's; the append then lands after it. Identity is asked again when
        // the wait ends, because a second restore may have replaced the object
        // meanwhile, and "is this the live room" can change while waiting.
        const replace = this.pendingLogReplace.get(entry.room);
        if (replace) {
  ...
  async restoreFromSnapshot(snapshot, { confirm } = {}) {
    ...
    // log waits for it instead of racing it. It is released only once the
    // replace is registered on the log's own tail (or the restore has been
    // abandoned), because that registration is what makes an append land after
    // the replace rather than be erased by it. See the flush in `#save`.
    const logReplace = { promise: null, release: null };
    logReplace.promise = new Promise((resolve) => { logReplace.release = resolve; });
    this.pendingLogReplace.set(restored, logReplace);
    try {
      await this.#save();
    } catch (error) {
      this.state.rooms[index] = previous;
      this.#releaseLogReplace(restored, logReplace);
      throw error;
    }
    ...
    // created against is the one now being replaced. The flush's identity check
    // drops every entry whose object this restore discarded — including one
    // re-queued by a failed write and re-claimed later — so no such append can
    // reach the restored log. What is left is an entry claimed for the restored
    // object, which is a legitimate append that must survive: this release is
    // what puts it after the replace instead of inside what the replace erases.
    try {
      const replaced = this.eventLog.replace(checked.snapshot.room.id, checked.snapshot.events);
      this.#releaseLogReplace(restored, logReplace);
      await replaced;
    } finally {
      this.#releaseLogReplace(restored, logReplace);
  ...
  /**
   * Release one restore's log-replace gate and drop it, so a flush waiting on it
   * proceeds. Idempotent, and it removes the registration only when it is still
   * this restore's: a second restore installs a different object, so the two
   * registrations are distinct keys and neither cleanup can clear the other's.
   */
  #releaseLogReplace(room, gate) {
    if (this.pendingLogReplace.get(room) === gate) this.pendingLogReplace.delete(room);
    gate.release();
  }
