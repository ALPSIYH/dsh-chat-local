...
export class DshChatLocalService {
  ...
  async setRoomPolicy(roomId, { defaultActionMode, expectedRevision, confirmRisk, gate }) {
    ...
    // so before this call could carry anything but a mode. The one call that
    // need not confirm is a change confined to the gate inside a mode the user
    // has already confirmed: that is not a permission change at all.
    const gateOnlyChange = gateChanged && !modeChanged && !presetMode;
    if (EXECUTION_MODES.has(mode) && confirmRisk !== true && !gateOnlyChange && (modeChanged || presetMode)) {
