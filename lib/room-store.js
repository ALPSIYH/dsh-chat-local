...
import { exportRoomSnapshot } from "./room-export.js";
import { workProtocol } from "./work-protocol.js";
...
const READ_ONLY_TOOL_NAME = /^(?:read|view|get|list|search|find|inspect|status|stat|query|fetch|web_search|web_fetch)(?:_|$)/iu;
const GROUP_INTERNAL_TOOLS = new Set(["chat_send", "chat_rooms", "chat_memory", "chat_charter_propose", "chat_charter_review", "chat_work", "chat_read_document", "chat_manage"]);
...
function isReadOnlyTool(name) {
  return GROUP_INTERNAL_TOOLS.has(name) || READ_ONLY_TOOL_NAME.test(name);
}
/**
 * Network-egress tools whose *name* looks read-only. A restricted group turn
 * may read, but it must not reach a local service: the plugin's own HTTP API
 * answers on loopback without credentials, so an unrestricted `web_fetch`
 * would let one room read every other room's full history and defeat exactly
 * the isolation the group chat promises.
 */
...
export class DshChatLocalService {
  ...
  #profileNotice(room, text, proposalId) {
    room.roomSeq += 1;
    room.messages.push({ id: crypto.randomUUID(), roomId: room.id, roomSeq: room.roomSeq,
  ...
  async setRoomPolicy(roomId, { defaultActionMode, expectedRevision, confirmRisk }) {
    await this.ready;
    const room = this.#room(roomId);
    const expected = Number(expectedRevision);
    ...
    if (EXECUTION_MODES.has(mode) && confirmRisk !== true) throw new Error("changing execution permissions requires explicit risk confirmation");
    if (mode === room.policy.defaultActionMode && !["full_access", "workspace_write"].includes(mode)) return roomView(room);
    if (["full_access", "workspace_write"].includes(mode)) {
    ...
    room.roomSeq += 1;
    room.messages.push({ id: crypto.randomUUID(), roomId, roomSeq: room.roomSeq, author: "system:policy", authorKind: "system", authorAlias: "权限设置", sentAt: Date.now(), actionMode: mode, policyRevision: room.policy.revision, mentions: [], deliveries: [],
      text: `用户将房间权限切换为 ${mode}。${mode === "full_access" ? "现有成员的原生 DSH 会话已设为 danger-full-access / never；允许命令与文件修改，不再额外审批。这些会话的原生设置也会保留在群聊之外。" : mode === "workspace_write" ? "现有成员原生 DSH 权限已设为 workspace-write / ask。" : mode === "inherit_dsh" ? "不改会话权限，由 DSH 原生沙箱与审批控制。" : "群聊恢复只读限制；不会静默重写原生会话权限。"} 不自动发送任务或宣称阻断已解除。` });
    this.#touchRoom(room);
    await this.#save();
    return roomView(room);
  }
  ...
  async prepareLedgerManagement(roomId, input, sessionId) {
    ...
    room.roomSeq++;
    room.messages.push({id:crypto.randomUUID(),roomId,roomSeq:room.roomSeq,author:"system:management",authorKind:"system",authorAlias:"协作整理",sentAt:Date.now(),mentions:[],deliveries:[],managementBatchId:batch.id,
  ...
  #commitLedger(room, current, patch, event) {
    ...
    if (event.actor !== "human:me" || (current && current.status !== next.status) || event.type === "restored") {
      if (!["comment","progress","handoff"].includes(event.type)) {
        ...
        room.messages.push({ id: crypto.randomUUID(), roomId: room.id, roomSeq: room.roomSeq, author: "system:work", authorKind: "system", authorAlias: "协作台账",
  ...
  guardToolExecution(execution) {
    ...
    if (EXECUTION_MODES.has(lock.actionMode)) return undefined;
  ...
  async #retryFailedDeliveries(roomId, messageId, sessionIds, options={}) {
    ...
    for(const continuation of continuations){
      ...
      for(const delivery of message.deliveries??[])if(delivery.status==="failed"&&continuation.retrySessionIds?.includes(delivery.member))Object.assign(delivery,{status:"superseded",supersededReason:"continued_with_current_policy",continuationMessageId:continuation.id});
    }
    ...
    if(options.mode==="current") {
      ...
      for(const delivery of message.deliveries)if(delivery.status==="failed"&&retryIds.includes(delivery.member))Object.assign(delivery,{status:"superseded",supersededReason:"continued_with_current_policy",continuationMessageId:next.id});
    ...
    for (const delivery of message.deliveries) {
      if (delivery.status === "failed" && retryIds.includes(delivery.member)) {
        ...
        delivery.supersededReason = "retried";
        delivery.retriedAt = now;
      }
    }
    room.epoch += 1;
  ...
  #supersede(room, nextEpoch) {
    ...
    for (const capture of [...this.pending.values()]) {
      ...
      this.#setDelivery(capture, "superseded", { completedAt: Date.now(), error: "superseded by a newer room message" });
  ...
  #participantPrompt(room, member, task) {
  ...
  async #runTurn(roomId, rootMessageId, epoch, recipientIds) {
    ...
    while (queue.length > 0 && visibleReplies < this.maxReplies) {
      ...
      const result = await this.#deliverOne(room, rootMessage, member, epoch, task);
  ...
  #newCapture(room, rootMessage, member, epoch, task, delivery) {
    ...
    capture.timer = setTimeout(() => {
      this.#setDelivery(capture, "failed", { completedAt: Date.now(), error: "Agent reply timed out" });
      try {
        if(capture.turn!==undefined && this.turnBySession.get(capture.sessionId)===capture.turn && this.policyLocks.get(capture.sessionId)?.captureId===capture.id)
  ...
  async #deliverOne(room, rootMessage, member, epoch, task) {
    const bridge = this.#bridge();
    const delivery = {
      id: crypto.randomUUID(),
    ...
    await this.#save();
    if(this.closed||room.epoch!==epoch){Object.assign(delivery,{status:"superseded",completedAt:Date.now(),error:"服务已关闭或回合已变化，未唤醒成员。"});await this.#save();return {status:"superseded"};}
    ...
    try {
      ...
      await bridge.deliverExternal(
        `room:${room.id}`,
        member.sessionId,
        this.#participantPrompt(room, member, task),
        { id: capture.id, transport: ROOM_TRANSPORT }
      );
      if (!capture.settled) {
        if (capture.delivery.status === "queued") this.#setDelivery(capture, "sent", { sentAt: Date.now() });
      ...
      this.#setDelivery(capture, "failed", { completedAt: Date.now(), error: message });
      await this.#save();
  ...
  #setDelivery(capture, status, fields = {}) {
    capture.delivery.status = status;
    Object.assign(capture.delivery, fields);
  }
  ...
  async observeSessionEvent(sessionId, event) {
    ...
    if (event?.type === "user/message") {
      ...
      this.#setDelivery(capture, "delivered", { deliveredAt: Date.now() });
      await this.#save();
      return;
    }
    ...
    if(event?.type==="turn/end"&&this.policyLocks.get(id)?.turn===turn&&this.policyLocks.get(id)?.ambiguous) {
      ...
      for(const item of mixed)this.#setDelivery(item,"failed",{completedAt:Date.now(),error});
      await this.#save();
    ...
    if (event.type === "assistant/message") {
      ...
      if (capture.delivery.status === "delivered" || capture.delivery.status === "sent") {
        this.#setDelivery(capture, "working", { startedAt: Date.now() });
        await this.#save();
      }
      return;
    }
    ...
    if (stale) {
      ...
      this.#setDelivery(capture, "failed", { completedAt: Date.now(), error });
      result = { status: "failed", error };
    } else if (capture.explicitRoomSend) {
      this.#setDelivery(capture, "replied", { completedAt: Date.now(), mode: "explicit" });
      result = { status: "replied", mode: "explicit", messages: capture.explicitMessages.map(copy) };
    } else if (passReply(capture.latestText)) {
      this.#setDelivery(capture, "passed", { completedAt: Date.now() });
      ...
      room.roomSeq = message.roomSeq;
      room.messages.push(message);
      this.#setDelivery(capture, "replied", { completedAt: Date.now(), mode: "automatic", replyMessageId: message.id });
