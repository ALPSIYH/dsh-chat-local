...
import { deriveRelationships, latestRelationships, effectiveAppraisals, revocableAppraisal, revocableInterval,
  ...
  APPRAISAL_CONFIDENCE_MIN, APPRAISAL_CONFIDENCE_MAX } from "./relationship.js";
 ...
 *
 * The counters are identical under every observer because they are read from the
 * log alone; the observer axis carries a member's own appraisals, which the
 * judgement block below renders as what they are.
 */
 ...
 * A claim's own text travels as a quotation under that heading, so it can be
 * read as a judgement and never as a system fact.
 */
const RELATIONSHIP_DIGEST_APPRAISAL_HEADING =
 ...
 * outside it is refused by the writer, and cannot reach here from a log a writer
 * wrote. An unrecognised value that a hand-built log does carry renders as its
 * own text rather than as an invented stance.
 */
const RELATIONSHIP_DIGEST_STANCES = new Map([
  ["trust", "信任"],
  ["distrust", "不信任"],
  ["neutral", "中立"]
]);

/** How much of one recall travels, so one claim cannot fill the whole budget. */
const RELATIONSHIP_DIGEST_CLAIM_MAX_CHARS = 120;
...
function digestAppraisalLine(target, label, appraisal, position) {
  ...
  const quoted = claim.length > RELATIONSHIP_DIGEST_CLAIM_MAX_CHARS
    ...
    : claim;
  const stance = RELATIONSHIP_DIGEST_STANCES.get(appraisal.stance) ?? String(appraisal.stance ?? "");
  ...
  const role = appraisal.perceivedRole ? `，他自认的角色「${digestLabel(appraisal.perceivedRole)}」` : "";
  return {
    rank: 2,
    position,
    target,
    text: `（判断）对「${label}」：${stance}，把握 ${confidence}，证据 ${evidence} 条${role}——他本人的说法「${quoted}」`
  };
}
...
export class DshChatLocalService {
  ...
  #participantPrompt(room, member, task, turnSample) {
    ...
    // turn share one sample, so what a member reads cannot contradict the
    // snapshot recorded for that turn. `null` (no readable state, or nothing
    // recorded yet) injects nothing at all rather than an empty heading.
    const aliases = new Map(room.members.map((item) => [item.sessionId, item.alias]));
    const digest = relationshipDigest({ derived: turnSample?.derived, observer: member.sessionId,
      appraisals: turnSample?.appraisals, labelOf: (sessionId) => aliases.get(sessionId) });
  ...
  async #deliverOne(room, rootMessage, member, epoch, task, turnSample) {
    ...
    try {
      ...
      const promptText = this.#participantPrompt(room, member, task, turnSample);
