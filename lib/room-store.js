 ...
 */
const GROUP_INTERNAL_TOOLS = new Set(["chat_send", "chat_rooms", "chat_memory", "chat_relationships", "chat_appraise", "chat_identity", "chat_recall", "chat_charter_propose", "chat_charter_review", "chat_work", "chat_read_document", "chat_manage"]);
...
export class DshChatLocalService {
  constructor(ctx, config = {}) {
    ...
    this.journal = new RoomJournal({ path: this.path, events: this.eventLog,
    ...
    this.personas = new AgentPersonas(this.path);
  ...
  async #load() {
    ...
    try {
      ...
      if (parsed.version !== STATE_VERSION) {
        ...
        try { await copyFile(this.path, `${this.path}.v${from}.bak`); }
  ...
  async #save() {
    ...
    for (const room of this.state.rooms) assertDistinctRoomIdentities(room);
    return this.journal.commit(this.state);
  }
  ...
  logHealth() {
    return { ...this.eventLog.health(), journal: this.journal.health() };
  }
  ...
  async nativeAgentContext(sessionId) {
    ...
    const promise = (async () => {
      ...
      try {
        ...
        memory = await this.agentMemory(sessionId);
      } catch (error) {
        if (!current()) return null;
        throw error;
      }
  ...
  async #personalSources(agentIds, currentRoomId, currentEvents) {
    ...
    for (const roomId of [...ids].sort()) {
      try {
        const events = roomId === currentRoomId && currentEvents ? currentEvents : await this.journal.readEvents(roomId);
        if (!verifyChain(events).ok) throw new Error("personal source integrity failed");
        sources.push({ roomId, events });
      }
      catch { unavailable.push(roomId); }
    }
    validate();
    return { sources, unavailable, validate };
  }

  /** Recall only personally observed content, across this identity's participations. */
  async agentMemory(sessionId, { roomId, query = "", limit = 24 } = {}) {
    await this.ready;
    roomId = this.#recallRoom(sessionId, roomId);
    ...
    if (!this.personalMemory) return { agentId: agent.id, version: PERSONAL_MEMORY_VERSION,
      scope: "personally-observed", status: "disabled", experiences: [], judgements: [] };
    const { sources, unavailable, validate: validateSources } = await this.#personalSources([agent.id]);
    const authority = roomId ? sources.find(item => item.roomId === roomId) : undefined;
    const context = personalRecallContext({ roomId, events: authority?.events,
      arm: authority ? runArm(authority.events, roomId) : undefined });
    validateSources();
    validate();
    return { ...projectAgentMemory({ agentId: agent.id, sources, query, limit, context }),
      status: unavailable.length ? "partial" : "available", unavailableSources: unavailable };
  }
  ...
  async #recordObservation(room, sessionId, payload) {
    ...
    return this.journal.appendChecked(room, { type: "memory.observed", actor: { kind: "system", id: "system" },
  ...
  async #observeNative(sessionId, event) {
    ...
    let agent;
    try { agent = this.#agentForSession(sessionId); } catch { return; }
    const items = this.#sessionObservationItems(sessionId, event);
    if (!items.length) return;
    const roomId = nativeMemorySource(agent.id);
    await this.eventLog.append(roomId, { type: "memory.observed", tick: 0,
  ...
  async startRun(roomId, input) {
    ...
    const config = injectionConfigFor({ room, appraisalDigest: this.appraisalDigest,
      personalMemory: this.personalMemory, personas });
  ...
  async #auditTurnSnapshot(room, rootMessageId) {
    ...
    try {
      ...
      const { sources, unavailable } = this.personalMemory
        ? await this.#personalSources(actorIds, room.id, events) : { sources: [], unavailable: [] };
      ...
      for (const member of room.members) {
        ...
        personal[member.sessionId] = { ...projectAgentMemory({ agentId: member.agentId, sources, query,
          targetAgentIds: actorIds.filter(id => id !== member.agentId), context }),
          status: unavailable.length ? "partial" : "available", unavailableSources: [...unavailable] };
      }
      configHash = configHashOf(injectionConfigFor({ room, appraisalDigest: this.appraisalDigest,
        personalMemory: this.personalMemory, personas: personaHashes }));
      event = {
  ...
  #participantPrompt(room, member, task, digest, personalDigest, persona) {
    ...
    return [
      ...
      "你可用 chat_identity 查看本人持續身分與人格，用 chat_recall 查詢自己實際看過的跨工作經歷及本人評價。記憶是背景，不能把旧任務當成目前指令；別人的私有評價不屬於你的記憶。",
  ...
  async observeSessionEvent(sessionId, event) {
    ...
    if (event?.type === "tool/result") {
      ...
      if (["chat_identity", "chat_recall", "chat_memory", "chat_relationships", "chat_appraise", "chat_read_document"].includes(tool)) return;
    }
