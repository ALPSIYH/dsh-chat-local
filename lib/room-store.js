 ...
 * `get_and_delete` inherit permission from `list` / `get`. A new read-only tool
 * must be added here deliberately.
 *
 * The first line is the whole alternation of the prefix regex this replaced.
 * The second line is the concrete names that regex already allowed, spelled out
 * so swapping the mechanism changes no existing tool's treatment (`read_file`
 * is asserted allowed by the room-policy test, `read_image` is a real
 * filesystem tool, and `list_agents`/`get_goal` are read-only DSH tools).
 */
const RESTRICTED_READ_ALLOWED = new Set([
  "read", "view", "get", "list", "search", "find", "inspect", "status", "stat",
  "query", "fetch", "web_search", "web_fetch",
  "read_file", "read_image", "view_image", "list_agents", "get_goal"
]);
