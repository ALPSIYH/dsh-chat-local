...
export class DshChatLocalService {
  ...
  async #auditTurnSnapshot(room) {
    ...
    try {
      ...
      // and the log carries what the reset closed over.
      if (runArm(events, room.id) === "reset_per_episode") {
        const reset = await this.#appendEpisodeReset(room, events, asOfTick);
   ...
   * glance that the room's own arm mechanism applied it rather than a human.
   ...
   */
  async #appendEpisodeReset(room, events, asOfTick) {
    ...
    return await this.#record(room, {
      ...
      payload: { action: "clear", observerId: null, targetId: null, counters: null, countersBefore,
        appliedBy: "arm", mechanism: "arm:reset_per_episode", note: null },
      provenance: { originClass: "system", sessionKind: "interactive" }
    });
  }
  ...
  async #runTurn(roomId, rootMessageId, epoch, recipientIds) {
    ...
    // injects one sample.
    const turnSample = await this.#auditTurnSnapshot(room);
