...
export class DshChatLocalService {
  ...
  /**
   * Record one delivery transition as a `delivery.sent` / `delivery.settled`
   * event. `previous` is passed in rather than read off the delivery so a caller
   * that has already applied the change (the restart recovery in `#load`) can
   * still report the status it replaced.
   *
  ...
  #setDelivery(room, capture, status, extra = {}) {
    ...
    return this.#recordDelivery(room, capture.rootMessageId ?? null, delivery, status, { ...extra, previous });
  }
