...
export class DshChatLocalService {
  constructor(ctx, config = {}) {
    ...
    this.pendingAudit = [];
    // The generation of each room's log, raised whenever a restore replaces that
    // log wholesale. An audit append claimed against the log that was there
    // before the replace must be dropped rather than land in the one that
    // replaced it: the state the event names is the state the replace discarded.
    this.logGeneration = new Map();
  ...
  async #save() {
    ...
    try {
      audited = claimed.map((entry) => ({
        entry,
        // The log each fact belongs to, read at claim time. A restore replaces a
        // room's log wholesale and raises its generation; see the flush below.
        generation: this.logGeneration.get(entry.room?.id) ?? 0,
    ...
    const flush = (async () => {
      for (const { entry, generation, inSnapshot } of audited) {
        ...
        // The room's whole log may have been replaced — a restore — while this
        // write was in flight. `inSnapshot` settled the state the snapshot holds
        // and `stillValid` settles the state that reached disk, but neither can
        // see the swap: the log these facts belong to is not the log now being
        // appended to, and the room id that names it is unchanged. An append here
        // would land in the restored log as an event no state in it explains, so
        // a flush claimed against a replaced log is a no-op.
        if ((this.logGeneration.get(entry.room?.id) ?? 0) !== generation) continue;
  ...
  async restoreFromSnapshot(snapshot, { confirm } = {}) {
    ...
    // Raise the room's log generation synchronously, before the replace is
    // queued. An audit flush still owed by a superseded turn was claimed against
    // the log being replaced, and this is what makes its append a no-op instead
    // of an orphan event in the restored log. The order is what makes it sound:
    // a flush that reads the generation before this line registers its append
    // before the replace below is chained behind it, so the replace erases that
    // append; a flush that reads it after drops the append outright.
    this.logGeneration.set(restored.id, (this.logGeneration.get(restored.id) ?? 0) + 1);
   ...
   * has landed: the room this save wrote must still be the live room, and the
   * delivery must still hang off one of that room's messages. The restore path
   * replaces the room object, and a message can be dropped before the write, so
   * a transition whose state is no longer in the room is not recorded.
   *
   * The status this event names is deliberately *not* part of this check. It was
   * settled when the save claimed the transition, against the snapshot it was
   * about to write, because the delivery object is mutated in place: by flush
