...
import { deriveRelationships, RELATIONSHIP_VERSION } from "./relationship.js";
...
export class DshChatLocalService {
   ...
   * own output, passed through untouched: no recomputation with other arguments,
  ...
  async #auditTurnSnapshot(room) {
    ...
    try {
      ...
      const derived = deriveRelationships({ events, roomId: room.id, asOfTick: room.tick ?? 0 });
      event = {
  ...
  async #runTurn(roomId, rootMessageId, epoch, recipientIds) {
    ...
    // snapshot cannot feed back into who runs.
    await this.#auditTurnSnapshot(room);
