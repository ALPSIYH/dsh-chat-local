...
export class DshChatLocalService {
  ...
  /** Split `"session:s2"` / `"human:me"` into the envelope's actor shape. */
  #actorOf(id) {
    const [kind, ...rest] = String(id ?? "system:system").split(":");
    return { kind, id: rest.length ? rest.join(":") : kind };
  }
  ...
  /** The transition facts a committed ledger entry carries. */
  #entryTransitionPayload(entry, event) {
    return this.#ledgerTransitionPayload({
      ...
      dispositionAction: entry.disposition?.action ?? entry.triage?.action ?? null
    });
  }
  ...
  #commitLedger(room, current, patch, event) {
    ...
    this.#auditLedgerTransition(room, this.#entryTransitionPayload(next, event), event.actor,
