...
export class DshChatLocalService {
  ...
  async agentIdentity(sessionId, roomId) {
    ...
    if (this.#agentForSession(sessionId, roomId).id !== agent.id) throw new Error("Agent identity changed while reading");
  ...
  async agentMemory(sessionId, { roomId, query = "", limit = 24 } = {}) {
    ...
    const { sources, unavailable } = await this.#personalSources([agent.id]);
    ...
    if (this.#agentForSession(sessionId, roomId).id !== agent.id) throw new Error("Agent identity changed while recalling");
    if (this.#recallRoom(sessionId, roomId) !== roomId) throw new Error("participant turn changed while recalling");
  ...
  async appraise(roomId, sessionId, input) {
    ...
    return action === "revoke"
      ? this.#revokeAppraisal(room, sessionId, input)
      : this.#recordAppraisal(room, sessionId, input);
  }

  /** Append one new appraisal statement. See `appraise` for the settled rules. */
  async #recordAppraisal(room, sessionId, input) {
    ...
    const written = await this.journal.appendChecked(room, event,
      () => this.#memoryMember(room, sessionId, true));
   ...
   */
  async #revokeAppraisal(room, sessionId, input) {
    const about = ensureText(input.aboutAgentId, "appraisal target");
    const tick = room.tick ?? 0;
  ...
  async prepareMember(roomId,sessionId) {
    ...
    if(existing){await existing;validate();return copy(member);}
    const promise=(async()=>{
  ...
  async restoreFromSnapshot(snapshot, { confirm } = {}) {
    ...
    return this.journal.runRoom(checked.snapshot.room.id, () => this.#restoreLocked(checked));
  }
