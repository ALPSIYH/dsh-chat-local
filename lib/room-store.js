...
export class DshChatLocalService {
  ...
  async #save() {
    ...
    // the live room — and the flush asks it for every entry itself, so no
    // `stillValid` has to restate it.
    ...
    const flush = (async () => {
      for (const { entry, inSnapshot } of audited) {
        ...
        // future event type has to restate it in its own `stillValid`: the
        // identity of the room object the fact belongs to. It is the only
   ...
   * asks whether the room object this fact belongs to is still the live one. A
   * future event type that forgets to restate the room in its `stillValid` is
   * still covered.
   */
  ...
  async restoreFromSnapshot(snapshot, { confirm } = {}) {
    ...
    // created against is the one now being replaced, so the flush's identity
    // check drops every entry whose object this restore discarded — including
    // one re-queued by a failed write and re-claimed later — and no such append
    // can reach the restored log.
