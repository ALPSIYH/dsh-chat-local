...
export class DshChatLocalService {
  ...
  logHealth() {
    return { ...this.eventLog.health(), journal: this.journal.health(), storage:this.capacity.health(),
      memory: { coverage: this.observationCoverage.health(), index: this.memoryIndex.stats(), policy: this.memoryLifecycle,
  ...
  async agentIdentity(sessionId, roomId) {
    ...
    validate();
    return { agentId: agent.id, alias: agent.alias, persona, memoryScope: "personally-observed" };
  }
  ...
  async #observeNative(sessionId, event) {
    ...
    if (this.closed || !this.personalMemory
      || this.policyLocks.get(sessionId)?.active || [...this.pending.values()].some(item => item.sessionId === sessionId)) return;
    let agent;
