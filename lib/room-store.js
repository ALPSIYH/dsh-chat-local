...
export class DshChatLocalService {
   ...
   *   envelope's own `id`, the `messageId` a `message.created` carries, the
   *   `entryId` a `ledger.transition` carries, and the `proposalId` a
   *   `charter.*` event carries — every identifier an agent can actually read
   *   back through `chat_memory`. An id that exists nowhere in this room's log
   *   is refused, so an appraisal cannot cite a conversation that never
   *   happened. Refusing (rather than dropping the unknown ids) is deliberate:
   *   silently narrowing a citation would record a different statement from the
   *   one the caller made.
   ...
   * payload identifiers the read-only surfaces hand to agents — a message id, a
   * ledger entry id and a charter proposal id.
   */
  async #knownEventIds(roomId) {
    ...
    for (const event of events) {
      ...
      if (typeof given.proposalId === "string" && given.proposalId) known.add(given.proposalId);
    }
    return known;
  }
