...
import { projectAgentMemory, renderPersonalMemory, nativeMemorySource, observedSessionItems, PERSONAL_MEMORY_VERSION } from "./agent-memory.js";
...
export class DshChatLocalService {
  ...
  async close() {
    ...
    await Promise.allSettled([...this.activeRuns, this.journal.writeTail]);
  ...
  async #personalSources(agentIds, currentRoomId, currentEvents) {
    ...
    for (const roomId of [...ids].sort()) {
      try {
        const events = roomId === currentRoomId && currentEvents ? currentEvents : await this.eventLog.read(roomId);
  ...
  async agentMemory(sessionId, { roomId, query = "", limit = 24 } = {}) {
    ...
    let context;
    if (roomId) {
      const events = sources.find(item => item.roomId === roomId)?.events ?? [];
      if (runArm(events, roomId) === "reset_per_episode") {
        ...
        context = { roomId, afterAt: reset?.at ?? Number.MAX_SAFE_INTEGER };
      }
    }
  ...
  #recordMessage(room, message) {
    return this.journal.appendChecked(room, this.#messageEvent(message), () => {
      if (!room.messages.some(item => item.id === message.id)) throw new Error("message was superseded before its audit could be recorded");
    });
  }
  ...
  async #auditTurnSnapshot(room, rootMessageId) {
    ...
    try {
      ...
      const reset = events.findLast(item => item.type === RELATIONSHIP_INTERVENTION_EVENT_TYPE
        && item.payload?.appliedBy === "arm" && item.payload?.memoryScope === "all");
      const context = arm === "reset_per_episode" ? { roomId: room.id, afterAt: reset?.at ?? Number.MAX_SAFE_INTEGER } : undefined;
  ...
  async #commitSend(messageId) {
    ...
    receipt.committing=(async()=>{
      ...
      if (message.authorKind === "session") await this.#recordObservation(room, message.author,
        { origin: "authored", messageIds: [message.id] });
  ...
  async #deliverOne(room, rootMessage, member, epoch, task, turnSample) {
    ...
    try {
      ...
      const modelAtDelivery = (await this.#runModels({ members: [member] }))[member.sessionId];
  ...
  async observeSessionEvent(sessionId, event) {
    ...
    if (event?.type === "user/message") {
      ...
      if (!capture) {
        ...
        if (!match) await this.#observeNative(id, event);
        return;
      }
