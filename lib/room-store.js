...
export class DshChatLocalService {
  constructor(ctx, config = {}) {
    ...
    // Messages awaiting their audit append; drained by the save that makes them
    // durable, so the log can never describe state that did not reach disk.
    this.pendingMessageAudit = [];
  ...
  async #save() {
    ...
    // flush below keeps a message queued during the write from being audited
    // by a save whose snapshot does not yet contain it.
    const audited = this.pendingMessageAudit.splice(0);
    ...
    try {
      ...
      this.pendingMessageAudit.unshift(...audited);
      throw error;
    }
    for (const { room, message } of audited) {
      // A message rolled back before it was persisted must never be audited.
      if (!room.messages.some((item) => item.id === message.id)) continue;
  ...
  /**
   * Queue a message's audit append. The log must never describe state that did
   * not reach disk, so the append waits for the save that persists it; this is
   * also what keeps the queue safe for synchronous writers whose caller saves.
   */
  #auditMessage(room, message) {
    this.pendingMessageAudit.push({ room, message });
  }
  ...
  #setDelivery(room, capture, status, extra = {}) {
    ...
    // The delivery's owning message is the root message the capture was created
    // for, so the chain message.created -> delivery.* is explicit rather than
    // left for a reader to reconstruct. A missing room is not fatal: the status
    // above is already applied and `#record` drops the unattributable append.
    ...
    return this.#record(room, status === "sent"
      ? { type: "delivery.sent", actor: { kind: "session", id: delivery.member },
          ...
          causes: messageId ? [messageId] : [],
          provenance: { originClass: "agent", sessionKind: "interactive", messageId } }
