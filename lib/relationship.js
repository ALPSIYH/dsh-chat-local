 ...
 *
 * - Membership comes from `member.added` / `member.removed` (R41). The roster
 *   inference this module used before those events existed survives only as the
 *   fallback for the part of a log that predates them, so an existing room still
 *   derives instead of returning empty; see `membershipFrom`.
 ...
 * (R41): membership is read from the log rather than inferred from who happened
 * to appear in a turn or a delivery. The inference is kept only as the fallback
 * for the part of a log written before those events existed — a whole log that
 * predates them, or the prefix of one that was already running when the writer
 * learned to emit them — so an existing room keeps deriving exactly what it
 * derived before the upgrade instead of coming back empty.
 *
 * The fallback's own sources are the ordered roster a turn executed, the
 * recipients it was scheduled for, and the members a delivery was addressed to.
 * Both turn lists are written from `room.members`, and a delivery can only be
 * addressed to a session a turn was scheduled for, so on a complete log the two
 * sources agree. The delivery source is kept anyway: an append is allowed to
 * fail without failing the room operation it observed, and a dropped
 * `turn.scheduled` must not erase the member whose delivery survived it. Its
 * cost is the over-inclusion R41 exists to remove — a session named only by a
 * stale delivery gets an all-zero observer row — which is why it no longer
 * covers the post-membership-event part of a log.
 *
 * A message author is deliberately not used on either path: an author is
...
function membershipFrom(sorted, tickCap) {
  const facts = sorted.filter((event) => statesMembership(event) && tickOf(event) <= tickCap);
  const basisIds = [];
  ...
  if (facts.length === 0) return { members: inferred(sorted), basisIds };

  // Everything before the first in-horizon membership event was written by a
  // log that could not state the fact, so its roster is still read.
  const firstFactAt = sorted.findIndex((event) => statesMembership(event) && tickOf(event) <= tickCap);
  const members = inferred(sorted.slice(0, firstFactAt));

  const lastAdd = new Map();
  const lastRemove = new Map();
  facts.forEach((event, index) => {
