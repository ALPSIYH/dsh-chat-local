...
/** A history has no event-count ceiling; never spread it into call arguments. */
export function maxEventTick(events) {
  return events.length === 0 ? 0
    : events.reduce((latest, event) => Math.max(latest, tickOf(event)), -Infinity);
}
...
export function deriveRelationships({ events, roomId, asOfTick } = {}) {
  ...
  const tickCap = asOfTick ?? maxEventTick(sorted);
