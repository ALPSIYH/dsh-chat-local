...
/** Tools this plugin registers: they act inside the room, and it enforces its own rules. */
const ROOM_READ_TOOLS = ["chat_memory", "chat_relationships", "chat_rooms", "chat_read_document"];
const ROOM_WORK_TOOLS = [
  "chat_create", "chat_join", "chat_invite", "chat_send", "chat_work", "chat_manage",
  "chat_charter_propose"
];
...
/**
 * What the table says about a tool it does not name. High impact is the
 * conservative reading of an unclassified effect, and it is the same stance the
 * restricted-turn read allowance takes: a tool has to be added deliberately.
 */
export const GATE_UNCLASSIFIED_TOOL = Object.freeze({ riskClass: "high", action: "execute" });
...
/**
 * Read one execution as `{ riskClass, action }`.
 *
 * A blank or missing tool name returns no classification at all, which the
 * judgement below turns into `deny`.
 */
export function classifyToolExecution(name, args) {
  ...
  return { riskClass: entry.riskClass, action: entry.action };
}
 ...
 */
export function evaluateGate({ pair, action, riskClass, policy } = {}) {
  ...
  if (action === "review" && counters.unresolvedDisagreements > 0) return "require_independent_review";
