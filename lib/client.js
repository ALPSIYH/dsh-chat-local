window.__ModuleLoader__.load({
  ...
  function PersonaEditor({agent,onClose,onSaved}) {
    ...
    return h(Dialog,{title:`${agent.alias??"Agent"} 的人格`,onClose:close,onEscape:close,busy:loading||busy,focusKey:loading||busy?"persona-waiting":"persona-ready"},
  ...
  return function WorkspaceComposer({initial,rosters=[],groups=[],onClose,onComplete,onRosterSaved,onDraftSaved,navigationToken}) {
    ...
    return h("section",{className:"dclStartWorkspace "+(isGroup?"dclGroupCreate":"dclTopicCreate"),"aria-label":isGroup?"建立群組":"新對話草稿"},
      ...
      function NativeContextNotice({ status, onOpen }) {
        if (!status || status.state === "available") return null;
        return h("div", { className: "dclAgentMeta", "data-native-context": status.state },
          h("p", null, status.message),
          status.state === "suppressed" ? h("button", { className: "dclAgentBtn", onClick: onOpen }, "開啟原生會話調整預設") : null
        );
      }
      ...
      return function ChatBody(props) {
        ...
        const openSession = async (participant) => {
          try {
            if (typeof ctx.uiWorkspace?.openSession !== "function") throw new Error("DSH 會話導航尚未可用，請重新載入頁面後再試。");
            if (participant.nativeSetup?.state === "pending") await prepareParticipant(participant);
            await ctx.uiWorkspace.openSession(participant.sessionId);
            props?.onClose?.();
          }
          catch (cause) { setError(cause.message ?? String(cause)); }
        };
                ...
                participants.length === 0 ? h("div", { className: "dclEmpty", style: { padding: "12px 0" } }, "暂无参与者。新建时只需名称，然后从 DSH 模型目录选择模型。") : participants.map((participant, index) =>
                  h("div", { key: participant.sessionId, className: "dclAgentRow", style: { alignItems: "flex-start" } },
                    ...
                    h("div", { className: "dclAgentInfo" },
                      ...
                      h(NativeContextNotice, { status: participant.nativeContext, onOpen: () => openSession(participant) }),
