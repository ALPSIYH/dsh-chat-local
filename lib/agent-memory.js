...
import { effectiveAppraisals, maxEventTick, MEMORY_CONTRACT_VERSION, RELATIONSHIP_INTERVENTION_EVENT_TYPE } from './relationship.js';
...
export function observedSessionItems(event) {
  ...
  if (event.type === 'tool/result') {
    ...
    const callId = source?.callId ?? message.toolCallId ?? blocks[0]?.toolCallId;
    if (typeof callId === 'string') result.toolCallId = callId;
    // V4 owns the outcome on the tool-role message; earlier hosts wrapped it
    // in a tool-result content block. An explicit current outcome wins.
    result.isError = typeof message.isError === 'boolean'
      ? message.isError : blocks.some(block => block.isError === true);
  }
  return [result];
}
...
export function projectAgentMemory({ agentId, sources, query = '', targetAgentIds = [], limit = 24, context } = {}) {
  ...
  for (const source of sources ?? []) {
    ...
    const interventions = [], byTarget = new Map();
    const horizon = maxEventTick(events.filter(event => event.type !== 'appraisal'));
    for (const event of events) {
      if (event.type === RELATIONSHIP_INTERVENTION_EVENT_TYPE) interventions.push(event);
      else if (event.type === 'appraisal' && event.payload?.observerAgentId === agentId) {
        const key = event.payload.targetAgentId ?? null;
        if (!byTarget.has(key)) byTarget.set(key, []);
        byTarget.get(key).push(event);
      }
    }
    // Only interval statements and interventions belong to A replay. Passing the
    // full source once per target previously multiplied unrelated history work.
    const projections = [...byTarget.values()].map(statements =>
      effectiveAppraisals([...interventions, ...statements], source.roomId, Math.max(horizon, maxEventTick(statements))));
...
export function renderPersonalMemory(memory, { maxChars = PERSONAL_MEMORY_MAX_CHARS, includeJudgements = true } = {}) {
  ...
  const candidates = [
    ...(memory?.beliefs ?? []).map(x => ({ ...x,
      line: `本人目前信念（待核查）：${bounded(x.claim, 120)}（證據 ${x.evidence?.length ?? 0}）`,
      shortened: clean(x.claim).length > 120 })),
  ...
  const available = memory?.totals ?? { experiences: memory?.experiences?.length ?? 0, judgements: memory?.judgements?.length ?? 0, beliefs: memory?.beliefs?.length ?? 0 };
  const count = value => Number.isFinite(value) && value >= 0 ? Math.floor(value) : 0;
  const total = Math.max(candidates.length, count(available.experiences) + count(available.judgements) + count(available.beliefs));
