...
export function apply(ctx, config = {}) {
  ...
  if (ctx.systemPrompt ?? ctx.get?.("systemPrompt")) {
    ctx.effect(() => ctx.on("system-prompt/assemble", async (assembly, context, next) => {
      ...
      return result;
    }), "dsh-chat-local: native Agent identity and personal memory");
  }
  ctx.effect(() => {
  ...
  ctx.tools.register({
    ...
    parameters: { type: "object", additionalProperties: false, properties: {
      ...
      limit: { type: "integer", minimum: 1, maximum: 100, description: "Maximum entries per memory category; defaults to 24." }
    } }, output: memoryOutput,
    async execute(args, exec) {
      const sessionId = owningSession(exec);
