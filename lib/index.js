...
export function apply(ctx, config = {}) {
  ...
  ctx.tools.register({
    ...
    description: "Read your own row of this room's relationship counters, read-only: for each member (including yourself) the deliveries addressed to them, the reviews you recorded on their work, their blocked reports and unresolved disagreements, their superseded charter proposals, and the messages they authored. Only your own row is returned; every other member's row for the same target is theirs. Room membership required. These counters are computed from public room events — deliveries, ledger transitions, messages, membership — so today every member's row for one target carries the same numbers: they describe what that target did in the room, not one agent's private opinion of them.",
    parameters: { type: "object", additionalProperties: false, properties: { room: { type: "string" } }, required: ["room"] },
    output: memoryOutput,
    async execute(args, exec) {
      const room = await service.resolveRoom(args.room);
