import { PERSON_CONTEXT_NAME } from "./host-compatibility.js";
...
export function apply(ctx, config = {}) {
  ...
  ctx.effect(() => ctx.on("session/event", (session, event) => {
    service.hostCompatibility.sessionEvent(String(session.id), event);
    void service.observeSessionEvent(String(session.id), event).catch(() => {});
  }), "dsh-chat-local: capture session replies");
  // Register an empty sentinel through the Host API. A preset suppressing
  // runtime contexts removes it before the waterfall; never bypass that policy
  // by pushing an unregistered context back into the returned assembly.
  const registerNativeContext = (promptCtx) => {
    const registry = promptCtx.get?.("systemPrompt") ?? promptCtx.systemPrompt;
    const canRegister = typeof registry?.context === "function";
    service.hostCompatibility.contextService(canRegister);
    const disposers = [];
    if (canRegister) disposers.push(registry.context({ name: PERSON_CONTEXT_NAME, order: 9000, text: "" }));
    disposers.push(promptCtx.on("system-prompt/assemble", async (assembly, context, next) => {
      service.hostCompatibility.assertPromptReady(context.agent);
      const admitted = canRegister && assembly.contexts.some(item => item.name === PERSON_CONTEXT_NAME);
      const result = await next();
      const sessionId = context.agent?.session?.id;
      if (sessionId === undefined || sessionId === null) return result;
      const index = result.contexts.findIndex(item => item.name === PERSON_CONTEXT_NAME);
      if (!canRegister) return result;
      service.hostCompatibility.observeContext(String(sessionId), admitted && index >= 0);
      if (!admitted || index < 0) return result;
      const text = await service.nativeAgentContext(String(sessionId));
      if (text) {
        // Persona Markdown is a literal value, never an interpolation template.
        result.variables.dsh_chat_person_context = text;
        result.contexts[index] = { name: PERSON_CONTEXT_NAME, text: "{{dsh_chat_person_context}}" };
      } else result.contexts.splice(index, 1);
      return result;
    }));
    return () => {
      for (const dispose of disposers.reverse()) dispose?.();
      service.hostCompatibility.contextService(false);
    };
  };
  if (typeof ctx.inject === "function") {
    ctx.inject(["systemPrompt"], promptCtx => {
      promptCtx.effect(() => registerNativeContext(promptCtx), "dsh-chat-local: native person context");
    });
    // The claim notification precedes prompt assembly. Changing presentation at
    // user/message would be too late: that step's schemas are already frozen.
    ctx.effect(() => {
      service.hostCompatibility.turnHooksMounted = true;
      const claimed = ctx.on("agent/inbox/claimed", payload => service.observeNativeInputClaim(payload));
      const disposed = ctx.on("agent/disposed", ({ agent }) => service.hostCompatibility.agentDisposed(agent));
      return () => { claimed?.(); disposed?.(); service.hostCompatibility.close(); };
    }, "dsh-chat-local: scoped group tool presentation");
        ...
        const parts = sub ? sub.split("/").map(decodeURIComponent) : [];
          if (req.method === "GET" && parts[0] === "health") {
            ...
            const status = audit.pendingRecovery || audit.journal.pendingOperations || audit.journal.checkpointPending || audit.journal.lastError || audit.journal.syncError
              ...
              : audit.memory?.coverage.failedReceipts || audit.memory?.reads.unavailableSourceCount
                || audit.memory?.maintenance.lastError || audit.memory?.index.evictedSources
                || audit.memory?.index.policyAuthorityMarkers || audit.memory?.index.policyHistoryIncomplete
                ? "memory_incomplete" : "ok";
            ...
            auditState.memory = {policy:memory.policy, captureIncomplete:memory.coverage.failedReceipts > 0,
              maintenanceError:memory.maintenance.lastError, sourceUnavailable:memory.reads.unavailableSourceCount > 0,
              indexIncomplete:memory.index.evictedSources > 0 || memory.index.policyAuthorityMarkers > 0 || memory.index.policyHistoryIncomplete,
              nativeContext:memory.nativeContext};
  ...
  // The Host validates the raw return value before output.render. Match the
  // JSON representation these tools already expose, omitting optional undefined
  // object fields while keeping errors as errors (never invent a top-level value).
  const registerTool = definition => ctx.tools.register({
    ...
    async execute(args, execution) {
      ...
      const encoded = JSON.stringify(value, function (key, item) {
        if (item === undefined && Array.isArray(this)) throw new TypeError(`tool "${definition.name}" returned an undefined array item`);
        if (typeof item === "number" && !Number.isFinite(item)
          || ["function", "symbol", "bigint"].includes(typeof item)) throw new TypeError(`tool "${definition.name}" returned a non-JSON value`);
        return item;
      });
      if (encoded === undefined) throw new TypeError(`tool "${definition.name}" returned no JSON value`);
