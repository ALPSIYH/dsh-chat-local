...
export function apply(ctx, config = {}) {
  ...
  ctx.tools.register({
    ...
    description: "Read your own row of this room's relationship counters, read-only. For each member (including yourself) it reports what the room's public events say about that member: the deliveries addressed to them and how those settled, the verdicts they recorded as reviewer, the blockers they reported and the ones later confirmed, the disputes left unresolved against them, the charter proposals of theirs that a later proposal replaced, and the messages they authored. Only your own row is returned; every other member's row is theirs. Room membership required. Because every counter comes from public room events — deliveries, ledger transitions, messages, membership — today every member's row for one target carries the same numbers: they describe what that target did in the room, not one agent's private opinion of them.",
    parameters: { type: "object", additionalProperties: false, properties: { room: { type: "string" } }, required: ["room"] },
    output: memoryOutput,
    async execute(args, exec) {
      const room = await service.resolveRoom(args.room);
