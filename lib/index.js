...
export function apply(ctx, config = {}) {
    ...
    return ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        const parts = sub ? sub.split("/").map(decodeURIComponent) : [];
          ...
          // The experiment's two mutating endpoints, and the only ones that are
          // human-only by construction: the service refuses a call whose
          // `appliedBy` is not exactly "human", and refuses any call while a
          // member of the room holds an active group-chat turn. No agent tool is
          // registered for either — an agent that could reset its own counters
          // would be rewriting both variables of the experiment. They share this
          // surface's one guard (`crossSiteRequest` above) with every other
          // mutating route.
  ...
  ctx.tools.register({
    name: "chat_appraise",
    description: "Record, or revoke, your own appraisal of another member of this room. An appraisal is a judgement, not a fact: it is stored as an `appraisal` event and never changes any relationship counter. `record` requires a stance (trust, distrust or neutral), a confidence between 0 and 1, a claim stating the judgement in your own words, and at least one evidence event id; every cited id must exist in this room's log (a `message.created` event's id or its messageId, a `ledger.transition` entryId — a charter proposal id is cited this way, since proposing emits a transition whose entryId is the proposal id — or an event envelope id; the only envelope id any tool hands back is the `appraisalId` that `chat_relationships` reports for your own current appraisal, which is citable), otherwise the whole call is refused rather than partially recorded. You may appraise only members of this room and never yourself. `revoke` retires the appraisal you currently have in force about one member: it appends a new event that closes the statement's interval and keeps the original record in the log — nothing is deleted — and requires an active participant turn.",
    parameters: { type: "object", additionalProperties: false, properties: {
      room: { type: "string" },
