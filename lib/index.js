...
function json(res, status, payload) {
  ...
  const etag=status===200&&res.dclRead?`"${createHash("sha256").update(body).digest("hex")}"`:undefined;
      ...
      handler: async (req, res) => {
        ...
        const parts = sub ? sub.split("/").map(decodeURIComponent) : [];
          if (req.method === "GET" && parts[0] === "health") {
            await service.ready;
            return sendOk(res, { name: "dsh-chat-local", version:"0.16.0-local.1", status: "ok" });
          }
