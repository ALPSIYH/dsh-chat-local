...
- **人类界面：`GET /api/dsh-chat-local/rooms/:id/relationships`** 返回**完整矩阵**。房间归用户所有，所以用户看得到每个观察者对每个对象的计数器。响应是 `{"ok": true, "value": {"观察者": {"对象": 计数器}}}`，也就是投影原样，不做重排：键就是 Session id，字段名就是计数器的名字，读的人不必先学一层本插件特有的包装。日志读不出来（行损坏、尾部被截断、日志路径不可读）时返回 `value: {}`，与「还没有任何快照」同形，而不是一个调用方无法处置的 500。
