...
除了 `rooms.json` 这份「当前状态」，每个对话还有一份**只追加的事件日志**：`~/.dsh/dsh-chat-local/events/<roomId>.jsonl`，一行一个事件，逐条用 SHA-256 串成链，同目录的 `<roomId>.jsonl.head` 记录该房间最后一个事件的哈希（头部锚点）。日志不是读模型，而是「当时到底发生了什么」的审计记录：事件的正文一旦写入就不再修改或重写（唯一的例外是从快照恢复——见下文，恢复会整份替换该房间的日志），房间被软删除或彻底移除也不会删掉它的日志。每个事件带版本号 `v`、`id`、`at`（epoch 毫秒）、`tick`、类型、`actor`、`payload`、`causes`、`provenance`、前一事件的 `prev` 与自身 `hash`。`at` 是**房间内严格递增的时间戳**：它的先后就是事件写入日志的先后，同一毫秒内连续追加的事件会逐条推后 1 毫秒（最多领先墙钟几毫秒，墙钟追上后自动收敛），所以 `at` 回答的是「在这个房间里谁先谁后」，而不是精确的墙钟读数。调用方显式给出的 `at` 原样保留；从快照恢复时，被重写的每个事件同样保留自己原有的 `at`。
...
**关系派生。** `lib/relationship.js` 是一个纯函数：只读事件日志，为房间内每一对「观察者 → 对象」（包含自己对自己）派生出十个计数器，两个派生结果**逐字节相同**——不读墙钟、不用随机数、不调用模型，事件先按 `(tick, at, id)` 排序再计数，因而可重算、可追溯、可审计。日志里的 `at` 在同一房间内严格递增，所以同一个房间内 `(tick, at)` 已经和追加顺序一致，`id` 只在读取不是本写入器产生的信封时才用作兜底。
