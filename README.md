...
**关系快照。** 每次排定一个回合，房间都会在排定之后追加一条 `relationship.snapshot`：`payload.pairs` 由 `deriveRelationships` 从**该事件之前**的全部日志派生而来，是那条纯函数原样给出的每个有向对（含计数器、该对用到的 `derivedFrom` 证据事件 id，以及该对最新证据的 tick）；`payload.version` 标出计数器定义的版本，`derivedFromCount` 是本次派生用到的全部事件条数，`asOfTick` 是取快照的那个回合 tick。快照只是一次观察——它由把该回合状态写进磁盘的那次保存来 flush，写入失败只计入健康计数、不会让回合失败；日志读不到时这一回合就不发快照，也同样不影响协作。成员顺序与集合在取快照之前就已确定，关系状态不会反过来影响调度。

**当前关系。** 「当前关系」是从日志读出的投影而不是另一份存储：`latestRelationships(events, roomId)` 扫描房间日志里的快照，返回一个按观察者、再按对象分组的 JSON 对象 `{"观察者": {"对象": 计数器}}`，每个有向对取**最近一条覆盖它**的快照里的计数器。日志里还没有任何快照（包括空日志）时返回 `{}`：投影只报告快照陈述过的事实，从不现场派生。观察者与计数器键按码点排序写入，因此同一份日志的投影不依赖事件的传入顺序，也不需要任何 `Map`/`Set` 的遍历顺序。
