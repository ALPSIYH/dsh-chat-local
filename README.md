...
计数器目前可以重算，也可以用 `latestRelationships` 从快照投影读到，但还没有通过工具或 HTTP 端点暴露，也不会改写任何既有事件。

**尚未提供**：带证据、可撤销的模型 appraisal 覆盖层与合并注入；默认关闭的治理门；跨 run 的实验脚手架（干预 API、run manifest、评测脚本）；把关系摘要注入成员提示词。本版也不提供保留期、轮转或日志裁剪工具。
